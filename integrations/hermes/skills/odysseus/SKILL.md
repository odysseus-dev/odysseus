---
name: odysseus
description: Use when the user asks Hermes Agent to read or write Odysseus data (todos, email, calendar, memory, documents) or to launch/monitor/stop a Cookbook model-serve task through the scoped Hermes Agent API. Requires ODYSSEUS_URL and ODYSSEUS_API_TOKEN.
---

# Odysseus

Use this skill when a user asks to interact with Odysseus from Hermes Agent.
...
- `ODYSSEUS_API_TOKEN`: Scoped API token created in Odysseus Settings > Integrations > Add Integration > Hermes Agent.

If either value is missing, do not guess credentials. Tell the user to create a Hermes Agent token in Odysseus Settings and expose both values to the terminal session.
...
```bash
python3 ~/.hermes/skills/odysseus/scripts/odysseus_api.py capabilities
python3 ~/.hermes/skills/odysseus/scripts/odysseus_api.py todos list
python3 ~/.hermes/skills/odysseus/scripts/odysseus_api.py todos add "Follow up"
```
...
```bash
python3 ~/.hermes/skills/odysseus/scripts/odysseus_api.py POST /api/codex/todos '{"action":"add","title":"Call dentist","due_date":"tomorrow at 5pm"}'
```
...
```bash
python3 ~/.hermes/skills/odysseus/scripts/odysseus_api.py emails list 5
python3 ~/.hermes/skills/odysseus/scripts/odysseus_api.py emails read UID
```

If `/api/codex/capabilities` does not show `email.read: true`, do not inspect email. Ask the user to enable Email read in the Hermes Agent settings.
...
```bash
python3 ~/.hermes/skills/odysseus/scripts/odysseus_api.py GET /api/codex/memory
python3 ~/.hermes/skills/odysseus/scripts/odysseus_api.py POST /api/codex/memory '{"text":"User prefers SI units","category":"preference"}'
```
...
python3 ~/.hermes/skills/odysseus/scripts/odysseus_api.py cookbook tasks

# Tail the failing one (sessionId from `cookbook tasks`)
python3 ~/.hermes/skills/odysseus/scripts/odysseus_api.py cookbook output serve-abc12345 400

# Stop the previous attempt before you try a new flag set
python3 ~/.hermes/skills/odysseus/scripts/odysseus_api.py cookbook stop serve-abc12345

# Relaunch with new flags. cmd MUST begin with one of the allowlisted binaries.
python3 ~/.hermes/skills/odysseus/scripts/odysseus_api.py cookbook serve \
...
If you are about to reach the Odysseus host/container, import app internals, query the database, or call MCP helper modules directly, stop. Those paths bypass Odysseus Settings and token scopes. Ask the user to enable the relevant Hermes Agent tool toggle instead.
