// ============================================
// Void UI — Main Application Orchestrator
...
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try { window.__voidDefaultChat = d; } catch (_) {}
      return d;
    }
  } catch (_) {}
  return null;
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'Void Chat';
  ...
  if (_opener) window._voidRouteOpener = _opener;
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('void-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  (function initModeToggle() {
    ...
    window.__voidSetChatMode = setMode;
  ...
  const SPLASH_COUNT_KEY = 'void-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_COMPACT_WIDTH ? 'Message...' : 'Message Void...');
      }
  ...
  if (incognitoBtn) {
    ...
    incognitoBtn.addEventListener('click', () => {
      ...
      if (chk.checked) {
        ...
        if (typeof window.__voidSetChatMode === 'function') {
          window.__voidSetChatMode('chat');
        } else {
        ...
        if (typeof window.__voidSetChatMode === 'function') {
          window.__voidSetChatMode(_restoreMode === 'chat' ? 'chat' : 'agent');
        }
  ...
  const UI_VIS_KEY = 'void-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'void-toolbar-visibility';
    try {
...
function startVoidApp() {
  if (window.__voidAppStarted) return;
  window.__voidAppStarted = true;
  const _bumpChatPriority = (ms = 10000) => {
    try {
      window.__voidChatBusyUntil = Math.max(window.__voidChatBusyUntil || 0, Date.now() + ms);
    } catch (_) {}
  };
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('void-doc-open-' + _curSession) === '1') {
  ...
  window._voidBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        ...
        if (window._voidRouteOpener) {
          try { window._voidRouteOpener(); } catch (_) {}
          window._voidRouteOpener = null;
        }
      });
  } else {
