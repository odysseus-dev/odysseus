// ============================================
// Juniperus UI — Main Application Orchestrator
...
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try { window.__juniperusDefaultChat = d; } catch (_) {}
      return d;
    }
  } catch (_) {}
  return null;
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'Juniperus Chat';
  ...
  if (_opener) window._juniperusRouteOpener = _opener;
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('juniperus-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  const SPLASH_COUNT_KEY = 'juniperus-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message Juniperus...');
      }
  ...
  const UI_VIS_KEY = 'juniperus-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'juniperus-toolbar-visibility';
    try {
...
// ============================================
function startJuniperusApp() {
  if (window.__juniperusAppStarted) return;
  window.__juniperusAppStarted = true;
  // Set CSS variables
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('juniperus-doc-open-' + _curSession) === '1') {
  ...
  window._juniperusBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        ...
        if (window._juniperusRouteOpener) {
          try { window._juniperusRouteOpener(); } catch (_) {}
          window._juniperusRouteOpener = null;
        }
      });
  } else {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startJuniperusApp, { once: true });
} else {
  startJuniperusApp();
}
