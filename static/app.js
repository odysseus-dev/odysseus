// ============================================
// Vita Machina UI — Main Application Orchestrator
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'Vita Machina Chat';
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message Vita Machina...');
      }
...
if (document.readyState === 'loading') {
  document.addEventListener("DOMContentLoaded", startVitaMachinaApp, { once: true });
} else {
  startVitaMachinaApp();
}
