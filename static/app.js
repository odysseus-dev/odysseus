...
// ============================================
// i18n runtime, preference wiring, and every central catalog must evaluate
// before modules whose import-time setup calls t().
...
async function _syncWelcomeModelHint() {
  ...
  if (hasModel) {
    if (sub && !sub.dataset.researchOrigText) sub.textContent = t('New chat ready.');
    if (tip) tip.textContent = t('Pick a model if you want, or just type.');
  } else {
    if (sub && !sub.dataset.researchOrigText) {
      sub.innerHTML = t('Welcome, {setupLink} to get started.', { setupLink: `<span class="setup-trigger-link" style="color:var(--accent,var(--red));font-weight:600;cursor:pointer;text-decoration:underline;" title="${t('Click to launch setup')}">${t('type /setup')}</span>` });
    }
    if (tip) tip.textContent = t('Add an AI endpoint from Settings in the sidebar, or paste an endpoint/API key into the chat.');
  }
}
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_COMPACT_WIDTH ? t('Message...') : t('Message Odysseus...'));
      }
