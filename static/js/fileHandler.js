...
export async function uploadPending(opts = {}) {
  ...
  try {
    ...
    if (uploaded.some((x) => x && x.gallery_id)) {
      try {
        localStorage.setItem("gallery-fresh-chat-upload", String(Date.now()));
      } catch (_) {}
      window.dispatchEvent(
        new CustomEvent("gallery-refresh", {
          detail: { source: "chat-upload" },
        }),
      );
    }
    console.log("[ATTACH] upload succeeded, clearing pendingFiles"); // Test log to verify that pendingFiles is cleared only on success
