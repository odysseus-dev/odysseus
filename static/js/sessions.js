...
export async function loadSessions() {
  try {
    ...
    if (hasPendingChat) {
      ...
      targetId = savedId;
    } else if (savedId && _groupChildSessions.has(String(savedId))) {
      targetId = savedId;
      ...
      for (const msg of msgHistory) {
        try {
          ...
          if (groupChildInfo && msg.role === 'assistant') {
            const childMeta = msg.metadata ? { ...msg.metadata, _fromHistory: true } : { _fromHistory: true };
            ...
            renderMsg = { ...msg, metadata: childMeta };
          }
