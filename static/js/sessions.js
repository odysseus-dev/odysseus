...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = "Hellaine's Jade Palace";
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Hellaine kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : "Hellaine's Jade Palace";
    }
