...
let _sortMode = Storage.get('juniperus-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Juniperus Chat';
...
const FOLDER_STATE_KEY = 'juniperus-folder-state';
const FOLDER_ORDER_KEY = 'juniperus-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Juniperus kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Juniperus Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('juniperus-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('juniperus-session-sort', mode);
  else Storage.remove('juniperus-session-sort');
  renderSessionList();
}
