  ...
  try {
    ...
    const _statusTag = (pkg, isLocal, isSystemDep, winBlocked) => {
      ...
      if (pkg.installed && pkg.pip_update_available === false) {
        const tip = esc(pkg.update_note || pkg.status_note || 'Found externally; update outside Vita Machina.');
    ...
    list.innerHTML = [
      _viewingRemote ? '' : _section('Vita Machina app', 'Run inside the Vita Machina app itself.', _appDeps),
...
function _renderRecipes() {
  ...
  html += '</select>';
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Optional packages that extend Vita Machina capabilities.</p>';
  ...
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Configure SSH servers, install Vita Machina keys, choose model directories, and set the default server. Local is this machine.</p>';
