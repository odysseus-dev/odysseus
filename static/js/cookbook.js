...
function _wireTabEvents(body) {
  ...
  if (dlBtn && dlInput) {
    ...
    const triggerDownload = async () => {
      ...
      // rather than discover the runaway download hours later. (Ollama names go
      // through `ollama pull`, not snapshot_download, so they skip this.)
      if (!ollamaName && !autoInclude && /gguf/i.test(repo)) {
