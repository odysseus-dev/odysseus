...
function _eventsForDay(dateStr) {
  return _events.filter(e => {
    ...
    // An event ending exactly at 00:00 (midnight) should NOT appear on
    // the day it technically ended on — CalDAV end-times are exclusive.
...
async function _renderMonth() {
  ...
  // Multi-day events render as inline dots on each day they cover — the
  // same path as single-day events. The old absolute-overlay "spanning bar"
  // rendering was removed because its CSS width math had boundary bugs
  // (endFrac=1 stretched events ending at 00:00 to cover the entire next
  // day) and broke the grid layout. (#4173)

  // Render 6 week rows.
  for (let row = 0; row < 6; row++) {
    h += `<div class="cal-week-row" style="--bars:0">`;
    ...
    for (let col = 0; col < 7; col++) {
      ...
      // All events for this day (single + multi-day) render as inline rows.
      const dayEvents = _eventsForDay(d);
      if (dayEvents.length) {
        const maxInline = window.innerWidth <= 768 ? 2 : 3;
        const showInline = dayEvents.slice(0, maxInline);
        ...
        if (dayEvents.length > maxInline) h += `<div class="cal-event-more">+${dayEvents.length - maxInline} more</div>`;
      }
      h += '</div>';
    }
    h += '</div>';
...
function _wireAll(body) {
  ...
  body.querySelectorAll('.cal-day[data-date]').forEach(cell => cell.addEventListener('click', (e) => {
    if (e.target.closest('.cal-event-item,.cal-event-row')) return;
