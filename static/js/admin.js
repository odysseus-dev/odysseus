...
async function _selectAddedModelInChat(endpoint) {
  ...
  try {
    document.dispatchEvent(new CustomEvent('juniperus:auto-select-model', {
      detail: {
...
function initEndpointForm() {
  ...
  document.querySelectorAll('#adm-add-api, #adm-add-local').forEach((sec) => {
    ...
    if (!head) return;
    const key = 'juniperus.addModels.' + sec.id + '.open';
    let open = false;
...
function initBackup() {
  el('adm-exportDataBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      const filename = match ? match[1] : 'juniperus_backup.json';
