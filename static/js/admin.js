...
function initDangerZone() {
  ...
  // DELETEs /api/admin/wipe/{kind}, and writes the result.
  const _LABELS = {
      ...
      try {
        if (isAll) {
          ...
          for (const k of kinds) {
            try {
              const { data } = await api.delete(`/api/admin/wipe/${k}`);
              results.push({ k, ok: true, count: data.count ?? 0, error: null });
            } catch (e) {
              const err = e.response?.data || {};
              results.push({ k, ok: false, count: 0, error: err.detail || e.message || 'failed' });
            }
          }
          ...
          try {
            ...
            const err = e.response?.data || {};
            if (_wipeMsg) { _wipeMsg.textContent = err.detail || 'Failed'; _wipeMsg.className = 'admin-error'; }
          }
        }
      } catch (e) {
