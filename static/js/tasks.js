...
function _renderList() {
  ...
  for (const task of visible) {
    ...
    const statusBadge = task.status === 'paused'
      ? `<button type="button" class="task-status-badge task-state-badge task-paused-badge" data-task-status-action="resume" title="${window.t('Paused - click to resume')}" style="position:relative;top:4px;"><svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="5" width="4" height="14" rx="1"/><rect x="14" y="5" width="4" height="14" rx="1"/></svg><span class="task-state-label">${window.t('paused')}</span></button>`
      : task.status === 'active'
        ? `<button type="button" class="task-status-badge task-state-badge task-active-badge" data-task-status-action="pause" title="${window.t('Active - click to pause')}" style="position:relative;top:4px;"><svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor"><polygon points="7 4 19 12 7 20 7 4"/></svg><span class="task-state-label">${window.t('active')}</span></button>`
        : '';
    ...
    <div class="task-form" style="flex:1;overflow-y:auto;min-height:0;">
      ...
      <label class="task-form-notif-toggle">
        ...
        <span class="task-form-notif-copy">
          ...
          <span>${window.t('Silence completion alerts for chatty cron jobs.')}</span>
        </span>
      </label>
      ...
      const syncActionExtra = async () => {
        ...
        if (action === 'check_email_urgency') {
          extra.insertAdjacentHTML('beforeend', `
            ...
            <div class="memory-desc" style="font-size:11px;margin-top:4px;">${window.t('Pause/resume and schedule are controlled by this task. It tags work, personal, urgent, action-needed, finance, legal, travel, newsletter, marketing, spam, and related mail categories. Urgent/reply-soon emails use your reminder settings.')}</div>
          `);
