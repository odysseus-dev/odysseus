...
export const ERROR_PATTERNS = [
  ...
  },
  {
    ...
    message: window.t('SGLang native kernel/runtime is missing or mismatched on this server.'),
    suggestion: window.t("Suggested action: relaunch with Odysseus’ venv CUDA library path fix. If the venv does not contain the matching NVIDIA runtime libs, run Repair sglang-kernel."),
    fixes: [
      { label: window.t('Edit / relaunch serve'), action: (panel) => _openServeEditFromDiagnosis(panel) },
      { label: window.t('Repair sglang-kernel'), action: (panel) => _repairSglangKernel(panel) },
      { label: window.t('Copy repair command'), action: (panel) => _copyText(_sglangKernelRepairCommand(panel)) },
...
export async function _runQuickCmd(panel, cmd) {
  ...
  if (diag) {
    diag.classList.remove('hidden');
    diag.innerHTML = '<div class="cookbook-diag-message">' + window.t('Running command...') + '</div>';
  }
  ...
  try {
    ...
    if (diag) {
      diag.innerHTML = ''
        + `<div class="cookbook-diag-message">${ok ? window.t('Command completed.') : window.t('Command failed.')}</div>`
        + `<div class="cookbook-diag-suggestion" style="opacity:0.75;margin-top:1px;">${window.t('Exit code: {code}', { code: _diagEsc(data.exit_code ?? 'unknown') })}</div>`
    ...
    if (diag) {
      diag.innerHTML = `<div class="cookbook-diag-message">${window.t('Command error.')}</div><div class="cookbook-diag-suggestion">${_diagEsc(e.message)}</div>`;
    }
  }
}
