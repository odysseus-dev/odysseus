...
export function wireAIToolsMisc({
  ...
  document.getElementById('ge-harmonize-run')?.addEventListener('click', () => {
    ...
    if (!body_mask) {
      if (uiModule) uiModule.showToast(t('editor.aiTools.toast.harmonizeNeedsSecondLayer'), 6000);
      return;
    }
  ...
  function canvasUpscale(factor) {
    ...
    fitZoom();
    composite();
    uiModule.showToast(t('editor.aiTools.toast.upscaledTo', { factor, width: newW, height: newH }));
  }
    ...
    try {
      ...
      if (data.image) {
        ...
        img.onload = () => {
          ...
          composite();
          renderLayerPanel();
          uiModule.showToast(t('editor.aiTools.toast.aiUpscaledTo', { width: newW, height: newH }));
        };
      ...
      uiModule.showToast(t('editor.aiTools.toast.aiUpscaleFailed', { error: e.message }));
    }
  ...
  document.getElementById('ge-style-run')?.addEventListener('click', async () => {
    ...
    if (!prompt) { uiModule.showToast(t('editor.aiTools.toast.enterStylePrompt')); return; }
    ...
    try {
      ...
      if (data.image) {
        ...
        img.onload = () => {
          ...
          composite();
          renderLayerPanel();
          uiModule.showToast(t('editor.aiTools.toast.styleApplied'));
        };
        img.src = 'data:image/png;base64,' + data.image;
      } else {
        throw new Error(data.error || 'No image returned');
      ...
      uiModule.showToast(t('editor.aiTools.toast.styleTransferFailed', { error: e.message }));
    }
