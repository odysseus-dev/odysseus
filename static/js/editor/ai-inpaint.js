...
export function wireInpaintButtons({
  ...
  async function runInpaint({ prompt, strength, btnId, labelId, idleLabel, busyLabel }) {
    ...
    if (!preMerged) { if (uiModule) uiModule.showToast(t('editor.inpaint.toast.drawAreaFirst')); return; }
    ...
    if (!hasMask) { if (uiModule) uiModule.showToast(t('editor.inpaint.toast.drawAreaFirst')); return; }
    ...
    try {
      ...
      resultImg.onload = () => {
        ...
        try {
          ...
          if (uiModule) uiModule.showToast(t('editor.inpaint.toast.completeDragEdgeControls'), 5000);
        } catch (renderErr) {
          console.error('[inpaint] render error', renderErr);
          if (uiModule) uiModule.showToast(t('editor.inpaint.toast.renderFailed', { error: renderErr.message || renderErr }), 6000);
        }
      };
      resultImg.onerror = (e) => {
        console.error('[inpaint] base64 decode failed', e);
        if (uiModule) uiModule.showToast(t('editor.inpaint.toast.resultDecodeFailed'), 6000);
      };
      resultImg.src = 'data:image/png;base64,' + data.image;
    } catch (e) {
      if (uiModule) uiModule.showToast(t('editor.inpaint.toast.failed', { error: e.message }), 6000);
    } finally {
      btn.disabled = false;
  ...
  document.getElementById('ge-inpaint-run').addEventListener('click', async () => {
    ...
    if (!prompt) { if (uiModule) uiModule.showToast(t('editor.inpaint.toast.enterPrompt')); return; }
  ...
  document.getElementById('ge-inpaint-outpaint').addEventListener('click', async () => {
    ...
    if (emptyCount === 0) {
      if (uiModule) uiModule.showToast(t('editor.inpaint.toast.noEmptyAreasForOutpaint'));
      return;
    }
    ...
    if (!mask) { if (uiModule) uiModule.showToast(t('editor.inpaint.toast.noActiveLayerForOutpaint')); return; }
