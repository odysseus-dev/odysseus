...
export function extractThinkingBlocks(text) {
  ...
  // Strip any remaining orphaned closing or opening think/thought tags
  cleanContent = cleanContent.replace(/<\/?(?:think(?:ing)?|thought)(?:\s+[^>]*)?>/gi, '');
  cleanContent = cleanContent.replace(/<\/?(?:mm:)?think(?:\s+[^>]*)?>/gi, '');
  cleanContent = cleanContent.replace(/<\|channel>(?:thought|response)\s*\n?/gi, '');
