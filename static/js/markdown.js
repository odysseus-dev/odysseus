...
// Fenced code blocks taller than this many lines render inside a collapsed
// <details> dropdown (like the thinking / bash-output sections) so a long
// wall of code can be skipped. Shorter snippets stay inline.
...
export function mdToHtml(src, opts) {
  ...
  s = s.replace(/```(\w+)?\n([\s\S]*?)```/g, (_, lang, code) => {
    ...
    const _pre = `<pre><code${langClass} data-lang="${lang || ''}">${escapeHtml(escaped)}</code>${runBtn}${editBtn}<button type="button" class="copy-code" data-code="${escapeHtml(escaped)}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg></button></pre>`;

    // Long code blocks get folded into a collapsible (like the thinking /
    // bash-output dropdowns) so a wall of code can be skipped. Collapsed by
    // default; short snippets stay inline. CODE_COLLAPSE_MIN_LINES is the
    // threshold.
    const _lineCount = escaped.split('\n').length;
    if (_lineCount > CODE_COLLAPSE_MIN_LINES) {
      const _label = (lang || 'code');
      codeBlocks.push(
        `<details class="code-collapse"><summary class="code-collapse-summary">`
        + `<span class="code-collapse-toggle"></span>`
        + `<span class="code-collapse-lang">${escapeHtml(_label)}</span>`
        + `<span class="code-collapse-meta">${_lineCount} lines</span>`
        + `</summary>${_pre}</details>`
      );
    } else {
      codeBlocks.push(_pre);
    }
