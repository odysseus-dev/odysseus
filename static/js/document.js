...
import { bindMenuDismiss, dismissOrRemove } from './escMenuStack.js';
  ...
  async function _attachOdysseusItem(kind, id, label, opts = {}) {
    try {
      ...
      if (uiModule) uiModule.showError('Failed to attach from Hellaine');
    }
  }
  ...
  async function _attachSelectedOdysseusItems(menu) {
    ...
    try {
      ...
      if (uiModule) uiModule.showError(added ? `Attached ${added}, then failed` : 'Failed to attach from Hellaine');
      _renderComposeAttachments();
    } finally {
      if (btn) {
