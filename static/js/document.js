...
import * as Modals from './modalManager.js';
  ...
  async function _resolveComposeSendAccountId() {
    const activeAccountId = window.__juniperusActiveEmailAccount || null;
  ...
  const _docOpenKey = (sessionId) => 'juniperus-doc-open-' + sessionId;
  const _docMinimizedKey = (sessionId) => 'juniperus-doc-minimized-' + sessionId;
    ...
    try {
      const res = await fetch(`${API_BASE}/api/email/draft`, {
        ...
        body: JSON.stringify({
          ...
          account_id: window.__juniperusActiveEmailAccount || null,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  export function openPanel() {
    ...
    let _fontIdx = parseInt(localStorage.getItem('juniperus-doc-fontsize') || '0', 10);
    ...
    function _applyDocFont() {
      ...
      localStorage.setItem('juniperus-doc-fontsize', _fontIdx);
    }
    _applyDocFont();
  ...
  const _DOC_RECENTS_KEY = 'juniperus-doc-actions-recent';
  ...
  function _saveSuggestionsToStorage() {
    ...
    if (data.length) {
      localStorage.setItem('juniperus-suggestions-' + activeDocId, JSON.stringify(data));
    } else {
      localStorage.removeItem('juniperus-suggestions-' + activeDocId);
    }
  }
  ...
  function _restoreSuggestionsFromStorage(docId) {
    try {
      const raw = localStorage.getItem('juniperus-suggestions-' + docId);
      if (!raw) return;
