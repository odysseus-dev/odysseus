  ...
  function _syncHeaderActions() {
    ...
    if (_copyBtn) {
      ...
      if (_replyable && _copyBtn.dataset.mode !== 'reply') {
        ...
        _copyBtn.title = t('Save new version');
        _copyBtn.innerHTML = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg>${_esc(t('Save'))}`;
      }
    }
  ...
  async function _aiReply(opts = {}) {
    ...
    try {
      ...
      if (data.success && data.reply) {
        ...
        if (uiModule) uiModule.showToast(t('AI draft inserted ({model})', { model: data.model_used || 'AI' }));
      } else {
        const rawMsg = data.error || t('Failed to generate reply');
        const msg = /empty response/i.test(rawMsg)
          ? t('AI reply failed: AI returned empty response.')
          : rawMsg;
  ...
  async function _scheduleSend(anchorEl = null) {
    ...
    overlay.querySelector('#sched-confirm').addEventListener('click', async () => {
      ...
      try {
        ...
        const data = await res.json();
        if (data.success) {
          if (uiModule) uiModule.showToast(t('Scheduled for {time}', { time: new Date(localDt).toLocaleString() }));
    ...
    pane.innerHTML = `
      ...
      <div class="doc-md-toolbar" id="doc-md-toolbar" style="display:none">
        <div class="md-toolbar-items" id="md-toolbar-items">
          <span class="md-view-toggle" id="doc-md-view-toggle" style="display:none" role="group" aria-label="${_esc(t('Edit or preview'))}">
            <button type="button" class="md-view-opt" data-mdview="edit" title="${_esc(t('Edit source (Ctrl+Alt+M to toggle)'))}"><span class="md-view-label">${t('Write')}</span><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.12 2.12 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
            <button type="button" class="md-view-opt" data-mdview="preview" title="${_esc(t('Preview (Ctrl+Alt+M to toggle)'))}"><span class="md-view-label">${t('Preview')}</span><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button>
          </span>
          ...
          <button type="button" class="md-toolbar-edit-only" data-md="bold" title="${_esc(t('Bold (Ctrl+B)'))}"><b>B</b></button>
          <button type="button" class="md-toolbar-edit-only" data-md="italic" title="${_esc(t('Italic (Ctrl+I)'))}"><i>I</i></button>
          <button type="button" class="md-toolbar-edit-only" data-md="strike" title="${_esc(t('Strikethrough'))}"><s>S</s></button>
          <span class="md-toolbar-sep md-toolbar-edit-only"></span>
          <button type="button" class="md-dd-toggle md-toolbar-edit-only" data-dd="heading" title="${_esc(t('Heading'))}"><b>H</b><svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg></button>
          <button type="button" class="md-dd-toggle md-toolbar-edit-only" data-dd="list" title="${_esc(t('List'))}"><span style="font-variant-numeric:tabular-nums;">1.</span><svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg></button>
          <span class="md-toolbar-sep md-toolbar-edit-only"></span>
          <button type="button" class="md-toolbar-edit-only" data-md="link" title="${_esc(t('Link'))}"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg></button>
          <button type="button" id="md-toolbar-attach-btn" class="md-toolbar-attach-btn md-toolbar-edit-only" title="${_esc(t('Insert image'))}"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m21.44 11.05-9.19 9.19a6 6 0 0 1-8.49-8.49l8.57-8.57A4 4 0 1 1 17.93 8.8l-8.59 8.57a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg></button>
          <button type="button" class="md-dd-toggle md-toolbar-email-hide md-toolbar-edit-only" data-dd="code" title="${_esc(t('Code'))}">\`<svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg></button>
          <span class="md-toolbar-sep md-toolbar-edit-only"></span>
      ...
      <div id="doc-actions-footer" class="doc-email-actions">
        <span class="email-send-split" id="doc-copy-export-split">
          <button type="button" id="doc-footer-copy-btn" class="email-send-btn email-send-main" title="${_esc(t('Save new version'))}" data-mode="save"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg>${_esc(t('Save'))}</button>
