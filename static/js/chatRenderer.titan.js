...
/**
 * Canonical Titan SD style labels — keep in sync with titan/style_labels.py.
 */
const TITAN_STYLE_LABELS = {
...
function normalizeImageModelLabel(model) {
  const raw = String(model || '').trim();
  if (/realvis\s*xl?/i.test(raw)) return TITAN_STYLE_LABELS.realistic;
  let m = raw.match(/^(realistic|anime|pixelart|krea)\b/i);
  if (m) return TITAN_STYLE_LABELS[m[1].toLowerCase()] || raw;
  m = raw.match(/style:\s*(realistic|anime|pixelart|krea)\b/i);
  if (m) return TITAN_STYLE_LABELS[m[1].toLowerCase()] || raw;
  if (raw.toLowerCase().startsWith('style:')) {
    const rest = raw.slice(6).trim();
    m = rest.match(/^(realistic|anime|pixelart|krea)\b/i);
    if (m) return TITAN_STYLE_LABELS[m[1].toLowerCase()] || rest;
  }
  return raw.split('/').pop() || 'image';
}

/**
 * Grid layout for multi-image batches (injected once; style.css may be read-only in deploy).
 */
function ensureGeneratedImageGridStyles() {
  if (document.getElementById('generated-image-grid-css')) return;
  const st = document.createElement('style');
  st.id = 'generated-image-grid-css';
  st.textContent = `
    ...
    .generated-image-batch .generated-image-cell { text-align: center; }
    .generated-image-batch .generated-image-cell .generated-image {
      max-height: 360px;
      width: 100%;
      object-fit: contain;
    }`;
...
/**
 * Normalize tool/history event into [{ url, id }, ...] for chat rendering.
 */
export function collectImageEntries(ev) {
  if (!ev || typeof ev !== 'object') return [];
  const urls = Array.isArray(ev.image_urls) && ev.image_urls.length
    ? ev.image_urls.filter(Boolean)
    : (ev.image_url ? [ev.image_url] : []);
  const ids = Array.isArray(ev.gallery_ids) && ev.gallery_ids.length
    ? ev.gallery_ids.filter(Boolean)
    : ((ev.image_id || ev.gallery_id) ? [ev.image_id || ev.gallery_id] : []);
...
/**
 * Build one or many generated-image bubbles (grid when batch).
 */
export function buildGeneratedImageNodes(ev) {
  const entries = collectImageEntries(ev);
  if (!entries.length) return [];
  const prompt = ev.image_prompt || '';
  const model = ev.image_model || '';
  const size = ev.image_size || '';
  const quality = ev.image_quality || '';

  if (entries.length === 1) {
    const e = entries[0];
    return [buildImageBubble(e.url, prompt, model, size, quality, e.id)];
  }

  const wrap = document.createElement('div');
  wrap.className = 'msg msg-ai generated-image-wrap generated-image-batch';
  ensureGeneratedImageGridStyles();
  const modelLabel = normalizeImageModelLabel(model);

  const role = document.createElement('div');
  role.className = 'role';
  role.textContent = modelLabel;
  ...
  for (const e of entries) {
    ...
    cell.className = 'generated-image-cell';
    if (e.id) cell.dataset.imageId = e.id;
    cell.dataset.imageUrl = e.url;

    const safeImageUrl = safeDisplayImageSrc(e.url);
    if (!safeImageUrl) {
      cell.textContent = '[Image unavailable]';
    } else {
      const img = document.createElement('img');
  ...
  parts.push(entries.length + ' images');
  const cost = getImageCost(model, quality, size);
  if (cost !== null) parts.push('$' + (cost * entries.length < 0.01
    ? (cost * entries.length).toFixed(4)
    : (cost * entries.length).toFixed(3)));
...
export function showWelcomeScreen() {
  ...
  const _incognitoChk = document.getElementById('incognito-toggle');
  const _researchChk = document.getElementById('research-toggle');
  if (!_incognitoChk?.checked && !(_researchChk && _researchChk.checked) && typeof window.applyTitanWelcomeBrand === 'function') {
      ...
      for (let r = 0; r < maxRound; r++) {
        ...
        if (roundTools.length > 0) {
          ...
          for (const ev of roundTools) {
            const isCard = !!(ev.image_proposal);
            const isWizard = !!(ev.wizard_pending);
            const isPending = isCard || isWizard;
            const ok = !isPending && (ev.exit_code === 0 || ev.exit_code == null);
            let outHtml = '';
            const _hideGenOutput = ev.tool === 'generate_image' && ok && (
              ev.image_url || (Array.isArray(ev.image_urls) && ev.image_urls.length)
            );
            if (ev.output && ev.output.trim() && !isCard && !_hideGenOutput) {
            ...
            node.className = 'agent-thread-node' + ((!ok && !isPending) ? ' error' : '') + (isPending ? ' proposal-pending' : '');
            ...
            const evIcon = isPending ? '\u25CB' : (ok ? '\u2713' : '\u2717');
            const evStatus = isWizard ? 'čeká' : (isCard ? 'návrh' : (ok ? 'done' : 'failed'));
            ...
            if (isCard && ev.image_proposal && window.__titanImageProposal) {
              const contentEl = node.querySelector('.agent-thread-content');
              if (contentEl) {
                window.__titanImageProposal.mountCard(contentEl, ev.image_proposal, {
                  onImage: (data) => {
                    for (const node of buildGeneratedImageNodes(data)) {
          ...
          for (const ev of roundTools) {
            for (const node of buildGeneratedImageNodes(ev)) {
              box.appendChild(node);
            }
          }
        }
