  ...
  for (let week = 0; week < 6 && day <= daysInMonth; week++) {
    ...
    for (let dow = 0; dow < 7; dow++, day++) {
      ...
      const badges = jobs.slice(0, 3).map(j => {
        ...
        const color = j.status === 'paused' ? '#888' : (j.status === 'completed' ? '#5a7' : '#5b9bd5');
...
function showDay(day) {
  ...
  document.getElementById('sched-daymodal').innerHTML =
    `<style>
      ...
      .sched-jobblock{background:var(--panel,#1e1e1e);border-left:3px solid #5b9bd5;border-radius:3px;padding:2px 6px;font-size:0.74rem;}
  ...
  host.innerHTML =
    `<div style="margin-top:10px;border:1px solid var(--border,#333);border-radius:6px;padding:14px;background:var(--panel,#161616);">` +
      ...
      `<div style="display:flex;gap:8px;margin-top:6px;flex-wrap:wrap;">` +
        ...
        (isEdit ? `<button id="je-delete" class="btn" style="padding:5px 12px;cursor:pointer;color:#d66;">Delete</button>` : '') +
...
function renderRuns(title, runs) {
  document.getElementById('sched-calendar').style.display = 'none';
  document.getElementById('sched-daymodal').innerHTML = '';
  const statusColor = s => ({ success: '#5a7', error: '#d55', running: '#5b9bd5', queued: '#aa7', skipped: '#888', aborted: '#a66' }[s] || '#888');
