...
const GROUP_STATE_KEY = 'void-group-state';
...
export async function startGroup(models, parentSessionId) {
  ...
  try {
    ...
    try {
      const storedGroupSessions = Storage.getJSON('void-group-sessions', []);
      const gids = Array.isArray(storedGroupSessions) ? storedGroupSessions : [];
      if (!gids.includes(_parentSessionId)) { gids.push(_parentSessionId); localStorage.setItem('void-group-sessions', JSON.stringify(gids)); }
    } catch (e) {}
  } catch (e) {
