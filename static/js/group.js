...
import * as fileHandlerModule from "./fileHandler.js";
...
function _initGroupTab() {
  ...
  async function _getModels() {
    ...
    items.forEach((item) => {
      ...
      (item.models || []).concat(item.models_extra || []).forEach((mid, i) => {
        ...
        seen.add(mid);
        const display =
          (item.models_display || []).concat(item.models_extra_display || [])[
            i
          ] || mid;
        result.push({
          mid,
          display: display.split("/").pop(),
          url: item.url,
  ...
  function _render() {
    ...
    _groupParticipants.forEach((p, idx) => {
      ...
      row.style.cssText =
        "display:flex;align-items:center;gap:6px;padding:4px 6px;background:color-mix(in srgb, var(--fg) 3%, transparent);border-radius:6px;";
      ...
      row.querySelector("button").addEventListener("click", () => {
        _groupParticipants.splice(idx, 1);
        _render();
      });
  ...
  addBtn.addEventListener("click", async () => {
    const [models, characters] = await Promise.all([
      _getModels(),
      _getCharacterList(),
    ]);
    ...
    modelSel.addEventListener("change", () => {
      ...
      if (_groupParticipants.length >= 8) {
        uiModule.showToast("Max 8");
        return;
      }
      ...
      if (charSel.value)
        entry.character =
          characters.find((c) => c.id === charSel.value) || null;
  ...
  if (modeBtn) {
    ...
    modeBtn.addEventListener("click", () => {
      ...
      modeBtn.innerHTML =
        (_mode === "parallel" ? ICON_PAR : ICON_SEQ) +
        '<span class="compare-toggle-label">' +
        (_mode === "parallel" ? "Parallel" : "Sequential") +
        "</span>";
    });
  }
  ...
  if (startBtn)
    startBtn.addEventListener("click", async () => {
      ...
      const _defaultModel =
        window.sessionModule && window.sessionModule.getSessions
          ? (() => {
              const s = window.sessionModule
                .getSessions()
                .find(
                  (x) => x.id === window.sessionModule.getCurrentSessionId(),
                );
              if (s)
                return {
                  mid: s.model,
                  display: s.model.split("/").pop(),
                  url: s.endpoint_url,
                  endpointId: "",
                };
              return null;
      ...
      const picked = _groupParticipants
        .map((p) => {
          let m = p.model
            ? { ...p.model }
            : _defaultModel
          ...
          if (p.character)
            m.character = {
              ...
              characterPrompt: p.character.prompt,
            };
          return m;
        })
        .filter(Boolean);
      ...
      if (picked.length < 2) {
        uiModule.showToast(
          "Need at least 2 participants — add models or characters",
        );
        return;
      }
      ...
      if (picked.length >= 2) {
        const presetData = {
          id: "grp-" + Date.now(),
          name: picked
            .map((p) => p._groupName || p.character?.characterName || p.display)
            .join(" & "),
          mode: _mode,
        ...
        try {
          const existing = await fetch(API_BASE + "/api/presets/groups", {
            credentials: "same-origin",
          }).then((r) => r.json());
          const groups = existing.groups || [];
          // Don't duplicate if same participants
          const sig = presetData.participants
      ...
      uiModule.showToast(
        "Group chat ready — " + picked.length + " participants",
      );
    });
  ...
  async function repopulateExistingSelections() {
    ...
    const characterSelections = participantsEl.querySelectorAll(
      "select.preset-input[data-selection-type=character]",
    );
    const modelSelections = participantsEl.querySelectorAll(
      "select.preset-input[data-selection-type=model]",
    );
    ...
    if (characterSelections.length !== 0) {
      ...
      characterSelections.forEach((characterSelection) => {
        ...
        const isChosenCharacterExisting =
  ...
  async function _loadGroupPresets() {
    try {
      const res = await fetch(API_BASE + "/api/presets/groups", {
        credentials: "same-origin",
      });
      ...
      groups.forEach((g, idx) => {
        ...
        chip.style.cssText =
          "padding:3px 10px;font-size:11px;background:color-mix(in srgb, var(--fg) 5%, transparent);border:1px solid var(--border);";
        ...
        chip.title = (g.participants || [])
          .map((p) => p.characterName || p.modelDisplay || "?")
          .join(", ");
        chip.addEventListener("click", async () => {
          ...
          (g.participants || []).forEach((p) => {
            ...
            if (p.characterId) {
              entry.character =
                chars.find((c) => c.id === p.characterId) || null;
            }
      ...
      console.warn("[group] Failed to load presets:", e);
    }
  }
...
async function _getCharacterList() {
  ...
  try {
    const r = await fetch(API_BASE + "/api/presets/templates", {
      credentials: "same-origin",
    });
    ...
    templates.forEach((t) => {
      if (t.id && t.name && !chars.find((c) => c.id === t.id)) {
        chars.push({
          id: t.id,
          name: t.name,
          prompt: t.system_prompt || t.prompt || "",
        });
      }
    });
  } catch (e) {}
...
export async function showModelPicker() {
  return new Promise((resolve) => {
    ...
    modeRow.style.cssText =
      "display:flex;gap:8px;margin-bottom:10px;align-items:center;font-size:12px;";
    modeRow.innerHTML = `
    ...
    async function getAllModels() {
      ...
      if (!items || items.length === 0) {
        try {
          ...
          console.warn("[group] Failed to fetch models:", e);
        }
      }
      const result = [];
      const seen = new Set();
    ...
    async function render(filter) {
      list.innerHTML =
        '<div style="opacity:0.4;padding:8px;font-size:12px;">Loading models…</div>';
      ...
      all.forEach((m) => {
        if (
          ...
          !m.epName.toLowerCase().includes(q)
        )
          return;
        const row = document.createElement("div");
        ...
        row.querySelector("input").addEventListener("change", (e) => {
          if (e.target.checked) {
            if (selected.size >= 8) {
              ...
              uiModule.showToast("Max 8 models");
              return;
            }
            selected.add(m.mid);
          } else {
          ...
          document.getElementById("group-selected-count").textContent =
            selected.size + " selected";
          document.getElementById("group-start-btn").disabled =
            selected.size < 2;
          row.style.background = selected.has(m.mid)
            ? "color-mix(in srgb, var(--accent, var(--red)) 12%, transparent)"
            : "";
        });
    ...
    // Start button
    document
      .getElementById("group-start-btn")
      .addEventListener("click", async () => {
        ...
        stepTitle.style.cssText =
          "font-size:12px;opacity:0.5;margin-bottom:8px;";
        ...
        `;
          ...
          sel.style.cssText =
            "font-size:11px;padding:3px 6px;border-radius:4px;border:1px solid var(--border);background:var(--bg);color:var(--fg);max-width:140px;";
        ...
        goBtn.style.cssText =
          "margin-top:10px;padding:6px 16px;font-size:12px;width:100%;";
...
export async function startGroup(models, parentSessionId) {
  ...
  const groupName =
    "[GRP] " +
    models
      .map((m) => m._groupName || m.character?.characterName || m.display)
      .join(", ");
  try {
    ...
    const pres = await fetch(`${API_BASE}/api/session`, {
      method: "POST",
      body: pfd,
      credentials: "same-origin",
    });
    ...
    try {
      const storedGroupSessions = Storage.getJSON(
        "odysseus-group-sessions",
        [],
      );
      const gids = Array.isArray(storedGroupSessions)
        ? storedGroupSessions
        : [];
      if (!gids.includes(_parentSessionId)) {
        gids.push(_parentSessionId);
  ...
  for (const m of models) {
    try {
      ...
      if (!res.ok) {
        console.error(
          `[group] Session creation failed for ${m.display}: HTTP ${res.status}`,
        );
      ...
      if (!data.id) {
        console.error(
          `[group] Session creation returned no ID for ${m.display}:`,
          data,
        );
        _participantSessions.push(null);
        continue;
      ...
      const otherNames = models
        .filter((x) => x.mid !== m.mid)
        .map((x) => (x.character ? x.character.characterName : x.display))
        .join(", ");
      ...
      if (m.character) {
        ...
        sysPrompt =
          `You are ${displayName} in a group chat with ${otherNames} and the user. ` +
          _groupEtiquette;
      }
      ...
      console.error(
        "[group] Failed to create participant session:",
        m.display,
        e,
      );
      _participantSessions.push(null);
  ...
  if (_parentSessionId && window.sessionModule) {
    ...
    try {
      history.replaceState(null, "", "#" + _parentSessionId);
    } catch (e) {}
...
export async function sendMessage(msg) {
  ...
  console.log(
    "[group] Pending attachment count:",
    fileHandlerModule.getPendingCount(),
  ); // Test log to verify the count of pending attachments

  const box = document.getElementById("chat-history");
  if (!box) return;

  // Upload pending attachments once for the whole group message.
  // fileHandler.uploadPending() also clears the composer attachment strip
  ...
  if (fileHandlerModule.getPendingCount()) {
    try {
      ...
      console.log("[group] Uploaded attachments:", attachmentIds);
    } catch (e) {
      console.error("[group] Attachment upload failed:", e);
      return;
    }
  }
        ...
        messages: [
          {
            ...
            ...(attachmentIds.length
              ? { metadata: { attachments: attachmentIds } }
              : {}),
          },
        ],
      }),
...
function _createGroupBubble(model, box) {
  ...
  const roleLabel =
    ...
    (model.character
      ...
      : chatRenderer.shortModel(model.mid));
  ...
  for (let turn = 0; turn < order.length; turn++) {
    ...
    if (response) {
      for (let j = 0; j < _participantSessions.length; j++) {
        ...
        try {
          await fetch(
            `${API_BASE}/api/session/${_participantSessions[j]}/inject_messages`,
            {
              method: "POST",
              credentials: "same-origin",
              headers: { "Content-Type": "application/json" },
...
async function _streamToHolder(
  modelIdx,
  sessionId,
  msg,
  holderEl,
  ...
  if (!sessionId) {
    holderEl.querySelector(".body").innerHTML =
      '<i style="opacity:0.5;">[Session creation failed]</i>';
    return;
  }
  ...
  if (attachmentIds.length) {
    fd.append("attachments", JSON.stringify(attachmentIds));
    console.log("[group] Attachments:", attachmentIds);
  }

  // Pass the active workspace to every group participant,
  // just like normal chat does.
  const _ws = (Storage.KEYS && Storage.get(Storage.KEYS.WORKSPACE, "")) || "";

  if (_ws) {
    fd.append("workspace", _ws);
    console.log("[group] Workspace:", _ws);
  }
  ...
  try {
        ...
        try {
          ...
          // Agent tool events
            ...
            toolDiv.style.cssText =
              "font-size:11px;opacity:0.5;padding:2px 0;font-family:monospace;";
            ...
            outDiv.style.cssText =
              "font-size:10px;opacity:0.4;padding:2px 0;font-family:monospace;max-height:60px;overflow:hidden;";
    ...
    bodyEl.innerHTML +=
      '<div style="color:var(--color-error);font-style:italic;">[Stream error]</div>';
  }
  ...
  if (accumulated) {
    ...
    !bodyEl.querySelector(".agent-tool-event") &&
