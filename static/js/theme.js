...
export function initThemeUI() {
  ...
  grid.innerHTML = Object.entries(THEMES).map(([name, c]) => `
    <div class="theme-swatch${name === activeName ? ' active' : ''}" data-theme="${name}">
      ...
      ${name === 'dark' ? window.t('original') : (name === 'gpt' ? 'GPT' : name)}
    </div>
  `).join('');
  ...
  if (customEntries.length > 0 && userGrid && userCard) {
    ...
    userGrid.innerHTML = customEntries.map(([name, c]) => `
      <div class="theme-swatch${name === activeName ? ' active' : ''}" data-theme="${name}" data-custom="1">
        ...
        <button type="button" class="theme-delete-btn" data-delete="${name}" title="${window.t('Delete theme')}"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
      </div>
    `).join('');
  } else if (userCard) {
    ...
    g.querySelectorAll('.theme-delete-btn').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        if (uiModule && uiModule.styledConfirm) {
          if (!await uiModule.styledConfirm(window.t('Delete theme "{name}"?', { name }), { confirmText: window.t('Delete'), danger: true })) return;
        }
  ...
  if (saveGoBtnOld && saveNameInputOld) {
    ...
    const doSave = () => {
      ...
      if (!name) { saveError.textContent = window.t('Enter a name.'); saveError.style.display = 'block'; return; }
      const slug = name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
      if (!slug) { saveError.textContent = window.t('Invalid name.'); saveError.style.display = 'block'; return; }
      if (THEMES[slug]) { saveError.textContent = window.t('Cannot overwrite a built-in theme.'); saveError.style.display = 'block'; return; }
      const colors = {};
      ...
      if (result === 'limit') { saveError.textContent = window.t('Max {n} custom themes. Delete one first.', { n: MAX_CUSTOM_THEMES }); saveError.style.display = 'block'; return; }
      save(slug, colors, opts);
      newNameInput.value = '';
      _flashAutosaved(window.t('Theme saved'));
  ...
  if (exportBtnEl) {
    ...
    newExp.addEventListener('click', () => {
      ...
      newExp.innerHTML = '&#x2713; ' + window.t('Downloaded!');
      setTimeout(() => { newExp.innerHTML = '&#x2913; ' + window.t('Export'); }, 1500);
    });
  }
  ...
  if (importGoEl && importAreaEl) {
    ...
    newGo.addEventListener('click', () => {
      ...
      catch { saveError.textContent = window.t('Invalid JSON.'); saveError.style.display = 'block'; return; }
      ...
      if (missing.length) { saveError.textContent = window.t('Missing: {list}', { list: missing.join(', ') }); saveError.style.display = 'block'; return; }
      const hexRe = /^#[0-9a-fA-F]{6}$/;
      for (const k of required) {
        if (!hexRe.test(colors[k])) { saveError.textContent = window.t('Bad hex for {k}', { k }); saveError.style.display = 'block'; return; }
      }
...
function _flashAutosaved(label = window.t('Auto-saved')) {
