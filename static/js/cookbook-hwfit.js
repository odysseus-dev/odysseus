...
function _renderHwVisibilityWarning(sys) {
  ...
  box.querySelector('[data-hw-action="copy"]')?.addEventListener('click', () => {
    ...
    const text = [
      'Void Cookbook hardware diagnostics',
      ...
      '',
      'Useful checks:',
      'docker compose exec void nvidia-smi -L',
      'docker compose exec void cat /proc/meminfo | head',
      'docker compose exec void python -c "from services.hwfit.hardware import detect_system; import json; print(json.dumps(detect_system(fresh=True), indent=2))"',
    ].join('\n');
...
export function _hwfitInit() {
  ...
  function _syncServers() {
    ...
    // dropdown still showed void. The user's selection must only change via
