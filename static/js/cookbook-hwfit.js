...
export async function _hwfitFetch(fresh = false, opts = {}) {
  ...
  if (_cached) {
    ...
    if (canKeepPrevious) {
      ...
      loadingTitle.textContent = window.t('No cached scan yet');
      loadingTitle.style.cssText = 'font-size:12px;opacity:0.7;';
      const loadingLbl = document.createElement('div');
      loadingLbl.textContent = window.t('Loading model list…');
    ...
    if (!canKeepPrevious) {
      ...
      const loadingLbl = document.createElement('div');
      loadingLbl.textContent = fresh ? window.t('Scanning hardware…') : window.t('Loading models…');
      loadingLbl.style.cssText = 'text-align:center;opacity:0.5;font-size:11px;';
      loadingDiv.appendChild(loadingLbl);
      setTimeout(() => {
        if (loadingLbl.isConnected) loadingLbl.textContent = fresh ? window.t('Scanning hardware…') : window.t('Loading model list…');
      }, 2000);
      list.innerHTML = '';
      list.appendChild(loadingDiv);
