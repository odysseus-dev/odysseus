...
async function refresh() {
  ...
  if (sEl) {
    sEl.innerHTML =
      ...
      `binary ${st.binary_provisioned ? '<span style="color:#5d5">provisioned ✓</span>' :
        '<span style="color:#d95">not provisioned</span>'}`;
  }
...
function renderServers(servers) {
  ...
  el.innerHTML = servers.map(s =>
    `<div style="display:flex;gap:8px;align-items:center;padding:4px 0;border-bottom:1px solid var(--border,#222);">` +
      ...
      `${s.alive ? '<span style="color:#5d5">alive</span>' : '<span style="color:#d55">dead</span>'} · ${s.uptime_s}s</span></span>` +
