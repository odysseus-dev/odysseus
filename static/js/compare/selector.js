...
async function showModelSelector() {
  return new Promise((resolve) => {
    ...
    const _selKey = 'juniperus-compare-selections-' + (state._compareMode || 'chat');
    let selections = Storage.getJSON(_selKey) || Storage.getJSON('juniperus-compare-selections') || [];
    // Restore synthesis models for search/research
    if (state._compareMode === 'search' || state._compareMode === 'research') {
      const savedSynth = Storage.getJSON('juniperus-compare-synth-' + state._compareMode);
