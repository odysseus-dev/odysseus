...
function _persistSelections() {
  if (state._selectedModels.length > 0) {
    Storage.setJSON('juniperus-compare-selections-' + (state._compareMode || 'chat'), state._selectedModels);
  }
  if ((state._compareMode === 'search' || state._compareMode === 'research') && state._searchSynthModels) {
    Storage.setJSON('juniperus-compare-synth-' + state._compareMode, state._searchSynthModels);
  }
}
...
const POOL_STORAGE_KEY = 'juniperus-shuffle-pool-excluded';
