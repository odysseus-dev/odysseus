...
function _openDetail(img) {
  ...
  detail.innerHTML = `
    ...
    <div class="gallery-detail-body">
      ...
      <div class="gallery-detail-sidebar">
        ...
        ${img.gen_seed != null ? `<div class="gallery-detail-section gallery-detail-provenance"><label>Generation</label><div class="gallery-detail-provenance-grid">
          <div><span class="gallery-prov-k">Seed</span> ${img.gen_seed}</div>
          ${img.steps != null ? `<div><span class="gallery-prov-k">Steps</span> ${img.steps}</div>` : ''}
          ${img.cfg_scale != null ? `<div><span class="gallery-prov-k">CFG</span> ${img.cfg_scale}</div>` : ''}
          ${img.sampler ? `<div><span class="gallery-prov-k">Sampler</span> ${_esc(img.sampler)}</div>` : ''}
          ${img.scheduler ? `<div><span class="gallery-prov-k">Scheduler</span> ${_esc(img.scheduler)}</div>` : ''}
          ${img.clip_skip != null ? `<div><span class="gallery-prov-k">Clip skip</span> ${img.clip_skip}</div>` : ''}
          ${img.quality ? `<div><span class="gallery-prov-k">Quality</span> ${_esc(img.quality)}</div>` : ''}
          ${img.size ? `<div><span class="gallery-prov-k">Size</span> ${_esc(img.size)}</div>` : ''}
        </div>
        ${img.negative_prompt ? `<div class="gallery-detail-neg"><span class="gallery-prov-k">Negative</span> ${_esc(img.negative_prompt)}</div>` : ''}
        </div>` : ''}
  ...
  if (window.__titanImageActions?.mountGalleryMenuItems) {
    window.__titanImageActions.mountGalleryMenuItems(menu, img, {
      onRefresh: (row) => _openDetail(row),
    });
  }
...
export function openGallery() {
    ...
    <div class="modal-content gallery-modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="gallery-toolbar">
          ...
          <button class="gallery-select-btn gallery-toolbar-action" id="gallery-folder-btn" title="Open the folder on disk where images are stored" style="position:relative;top:2px;font-size:12px;padding:10px 11px 12px;"><span style="position:relative;top:1px;display:inline-flex;align-items:center;gap:5px;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 20h16a2 2 0 0 0 2-2V8a2 2 0 0 0-2-2h-7.93a2 2 0 0 1-1.66-.9l-.82-1.2A2 2 0 0 0 7.93 3H4a2 2 0 0 0-2 2v13c0 1.1.9 2 2 2Z"/></svg>Folder</span></button>
  ...
  // ── Open the on-disk folder where generated images are stored ──
  // The backend tries to launch the host file manager; inside a headless
  // container that's a no-op, so we fall back to copying the path.
  document.getElementById('gallery-folder-btn')?.addEventListener('click', async () => {
    try {
      const r = await fetch(`${API_BASE}/api/gallery/folder`, { credentials: 'same-origin' });
      const d = await r.json().catch(() => ({}));
      if (!r.ok || !d.ok) throw new Error(d.detail || `HTTP ${r.status}`);
      if (d.opened) {
        uiModule.showToast('Opened gallery folder');
      } else {
        let copied = false;
        try { await navigator.clipboard.writeText(d.path); copied = true; } catch (_) {}
        uiModule.showToast((copied ? 'Path copied — ' : '') + 'Gallery folder: ' + d.path, 6000);
      }
    } catch (e) {
      uiModule.showError('Could not open gallery folder: ' + (e.message || e));
    }
  });
  ...
  function _showGalleryBulkMenu(anchor) {
    ...
    // Body-level menu (not `.dropdown` — that class is position:absolute with
    // z-index:1000 and can render behind tool modals whose stack climbs past
    // 10001). Match documentLibrary/tasks: sit above the top tool window.
    const dropdown = document.createElement('div');
    dropdown.className = 'gallery-bulk-menu';
    const rect = anchor.getBoundingClientRect();
    const left = Math.min(rect.left, window.innerWidth - 200);
    dropdown.style.top = `${rect.bottom + 6}px`;
    dropdown.style.left = `${Math.max(8, left)}px`;
    dropdown.style.setProperty('z-index', String(Math.max(topToolWindowZ() + 10, 100010)), 'important');
    ...
    if (mr.bottom > window.innerHeight - 6) {
      dropdown.style.top = `${Math.max(6, rect.top - mr.height - 6)}px`;
    }
    if (mr.right > window.innerWidth - 6) {
      dropdown.style.left = `${Math.max(6, window.innerWidth - mr.width - 6)}px`;
    }
    const close = (ev) => {
