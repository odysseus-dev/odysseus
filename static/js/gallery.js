...
function _renderGrid() {
  ...
  _items.forEach(img => {
    ...
    const fallbackName = (img.filename || '')
      ...
      .replace(/[_-]+/g, ' ')
      .trim();
    const labelText = (img.caption || '').trim() || (img.prompt || '').trim() || fallbackName || window.t('Photo');
...
function _openDetail(img) {
  ...
  detail.innerHTML = `
    <div class="gallery-detail-header">
      ...
      <button class="gallery-detail-back" id="gallery-chat-photo-btn" title="${img.session_id ? window.t('Open source chat') : window.t('Start a new chat with this photo')}" aria-label="${img.session_id ? window.t('Open source chat') : window.t('Discuss photo')}" style="display:inline-flex;align-items:center;gap:4px;">
        ...
        ${img.session_id ? window.t('Open chat') : window.t('Discuss')}
      </button>
    ...
    <div class="gallery-detail-body">
      ...
      <div class="gallery-detail-sidebar">
        ...
        ${img.caption ? `<div class="gallery-detail-section"><label>${window.t('OCR Caption')}</label><div class="gallery-detail-prompt">${_esc(img.caption)}</div></div>` : ''}
