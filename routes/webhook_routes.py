...
"ModelEndpoint", ...)`` pattern used by test_null_owner_gates.py all operate
on the *same* object the application actually uses. Keeps existing import
paths working after slice 2l (#4082/#4071). Source-introspection tests
(spec_from_file_location in test_api_chat_security.py) read the canonical
file by path.
"""
