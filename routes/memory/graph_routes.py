...
def setup_graph_routes(memory_manager, memory_vector, warm_router=None,
    ...
    # Explicit full paths (no router prefix): this keeps route registration
    # version-agnostic. A router-level prefix exercises a known upstream
    # include_router path that changed behaviour between FastAPI versions;
    # explicit paths sidestep it, so the memory feature does NOT require
    # pinning/downgrading FastAPI globally.
    router = APIRouter(tags=["memory-brain"])

    @router.get("/api/memory-brain/overview")
    def get_brain(request: Request) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning("brain snapshot error: %s", e)
    ...
    @router.get("/api/memory-brain/pressure")
    def get_pressure() -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning("pressure endpoint error: %s", e)
            return {"available": False, "score": None,
                    "note": "pressure unavailable"}

    @router.post("/api/memory-brain/sleep")
    def run_sleep() -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning("sleep endpoint error: %s", e)
            return {"ran": False, "reason": "consolidation failed",
                    "receipt": None}
