    ...
    async def model_serve(request: Request, req: ServeRequest):
        ...
        if is_windows and remote:
            ...
            if ("llama_cpp" in req.cmd or "llama-server" in req.cmd) and not local_windows_llama_cmd:
                ...
                runner_lines.append('    # Best-effort: huggingface_hub knows the platform default')
                runner_lines.append('    # (macOS uses ~/Library/Caches/huggingface, not ~/.cache) - #5978.')
                runner_lines.append('    try:')
                runner_lines.append('        import huggingface_hub')
                runner_lines.append('        _hc = getattr(getattr(huggingface_hub, "constants", None), "HF_HUB_CACHE", None)')
                runner_lines.append('        if _hc: add(_hc)')
                runner_lines.append('    except Exception:')
                runner_lines.append('        pass')
                runner_lines.append('    hf_home = os.environ.get("HF_HOME")')
                runner_lines.append('    if hf_home: add(os.path.join(hf_home, "hub"))')
                runner_lines.append('    if sys.platform == "darwin":')
                runner_lines.append('        add("~/Library/Caches/huggingface/hub")')
                runner_lines.append('    else:')
                ...
                runner_lines.append('    ODYSSEUS_PREFLIGHT_EXIT=127')
                runner_lines.append('  fi')
                # Reset any lingering MLX environment variables before each run to ensure a clean environment
                runner_lines.append('    # Begin MLX env reset')
                runner_lines.append('    for var in MLX_ROOT MLX_CACHE; do')
                runner_lines.append('        if [ -n "${!var}" ]; then')
                runner_lines.append('            echo "[odysseus] Unsetting $var for clean MLX run"')
                runner_lines.append('            unset $var')
                runner_lines.append('        fi')
                runner_lines.append('    done')
                runner_lines.append('    # End MLX env reset')
