...
def setup_cookbook_routes() -> APIRouter:
    ...
    async def model_download(request: Request, req: ModelDownloadRequest):
        ...
        if remote and is_windows:
            ...
            runner_lines.append('if [ $_ody_exit -eq 0 ]; then echo ""; echo "DOWNLOAD_OK"; else echo ""; echo "DOWNLOAD_FAILED (exit $_ody_exit)"; fi')
            ...
            if IS_WINDOWS:
                ...
                lines.append(hf_cmd)
                lines.append('_ody_exit=$?')
                lines.append('if [ $_ody_exit -eq 0 ]; then echo ""; echo "DOWNLOAD_OK"; else echo ""; echo "DOWNLOAD_FAILED (exit $_ody_exit)"; fi')
            else:
                # < /dev/null suppresses interactive "update available? [Y/n]" prompt
        ...
        if is_windows and remote:
            ...
            if "llama_cpp" in req.cmd or "llama-server" in req.cmd:
                ...
                if local_windows:
                    ...
                    runner_lines.append(f"  {_pip_install_fallback_chain('llama-cpp-python', python_cmd='python')} || true")
                    runner_lines.append('fi')
                    runner_lines.append('if ! command -v llama-server &>/dev/null && ! python -c "import llama_cpp" 2>/dev/null; then')
            ...
            if not handled_ollama_serve:
                ...
                if local_windows:
                    ...
                    _append_serve_exit_code_lines(runner_lines, keep_shell_open=False, is_pip_install=is_pip_install)
                else:
                    # Keep shell open after exit so user can see errors
                    _append_serve_exit_code_lines(runner_lines, keep_shell_open=True, is_pip_install=is_pip_install)
