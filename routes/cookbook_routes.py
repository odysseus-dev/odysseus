    ...
    async def model_serve(request: Request, req: ServeRequest):
        ...
        if is_windows and remote:
            ...
            if "llama_cpp" in req.cmd or "llama-server" in req.cmd:
                ...
                runner_lines.append('export ODYSSEUS_LLAMA_CPP_DIR="${ODYSSEUS_LLAMA_CPP_DIR:-$HOME/.local/llama.cpp}"')
                runner_lines.append('export ODYSSEUS_LLAMA_CPP_BIN_DIR="${ODYSSEUS_LLAMA_CPP_BIN_DIR:-$HOME/.local/bin}"')
                runner_lines.append('export PATH="$ODYSSEUS_LLAMA_CPP_BIN_DIR:$HOME/.local/bin:$HOME/bin:$ODYSSEUS_LLAMA_CPP_DIR/build/bin:/opt/homebrew/bin:/usr/local/bin:$PATH"')
