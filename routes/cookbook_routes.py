        ...
        if is_windows and remote:
            ...
            if "llama_cpp" in req.cmd or "llama-server" in req.cmd:
                ...
                if local_windows:
                    ...
                    runner_lines.append('  else')
                    runner_lines.append('    # Auto-detect GPU and configure toolkit paths')
                    runner_lines.append('    GPU_TYPE="CPU"')
                    runner_lines.append('    if [ -e /dev/nvidiactl ] || [ -e /dev/nvidia0 ]; then')
                    ...
                    runner_lines.append('      }')
                    runner_lines.append('      if command -v nvcc &>/dev/null && _odysseus_has_cudart; then')
                    runner_lines.append('        GPU_TYPE="CUDA"')
                    runner_lines.append('        CUDA_BIN=$(command -v nvcc)')
                    runner_lines.append('        CUDA_ROOT=$(dirname $(dirname "$CUDA_BIN"))')
                    runner_lines.append('        export PATH="$CUDA_ROOT/bin:$PATH"')
                    runner_lines.append('        export LD_LIBRARY_PATH="$CUDA_ROOT/lib64:$CUDA_ROOT/extras/CUPTI/lib64:$LD_LIBRARY_PATH"')
                    runner_lines.append('      else')
                    runner_lines.append('        echo "[odysseus] WARNING: GPU device detected but CUDA toolchain is incomplete — falling back to CPU build."')
                    runner_lines.append('      fi')
                    runner_lines.append('    elif [ -e /dev/kfd ]; then')
                    # ROCm: verify hipcc exists before enabling HIP
                    runner_lines.append('      if command -v hipcc &>/dev/null; then')
                    runner_lines.append('        GPU_TYPE="ROCm"')
                    runner_lines.append('        ROCM_BIN=$(command -v hipcc)')
                    runner_lines.append('        ROCM_ROOT=$(dirname $(dirname "$ROCM_BIN"))')
                    runner_lines.append('        export PATH="$ROCM_ROOT/bin:$PATH"')
                    runner_lines.append('        export LD_LIBRARY_PATH="$ROCM_ROOT/lib:$LD_LIBRARY_PATH"')
                    runner_lines.append('      else')
                    runner_lines.append('        echo "[odysseus] WARNING: GPU device detected but hipcc not found — falling back to CPU build."')
                    runner_lines.append('      fi')
                    runner_lines.append('    elif [ -e /dev/dri/renderD128 ]; then')
                    # SYCL: verify icpx exists before enabling SYCL
                    runner_lines.append('      if command -v icpx &>/dev/null; then')
                    runner_lines.append('        GPU_TYPE="SYCL"')
                    runner_lines.append('        SYCL_BIN=$(command -v icpx)')
                    runner_lines.append('        SYCL_ROOT=$(dirname $(dirname "$SYCL_BIN"))')
                    runner_lines.append('        export PATH="$SYCL_ROOT/bin:$PATH"')
                    runner_lines.append('        export LD_LIBRARY_PATH="$SYCL_ROOT/lib:$LD_LIBRARY_PATH"')
                    runner_lines.append('      else')
                    runner_lines.append('        echo "[odysseus] WARNING: GPU device detected but icpx not found — falling back to CPU build."')
                    runner_lines.append('      fi')
                    runner_lines.append('    fi')
                    runner_lines.append('    echo "Detected GPU: $GPU_TYPE"')
                    runner_lines.append('    CMAKE_FLAGS="-DGGML_CUDA=OFF -DGGML_HIPBLAS=OFF -DGGML_SYCL=OFF"')
                    runner_lines.append('    if [ "$GPU_TYPE" = "CUDA" ]; then')
                    runner_lines.append('      CMAKE_FLAGS="-DGGML_CUDA=ON"')
                    runner_lines.append('    elif [ "$GPU_TYPE" = "ROCm" ]; then')
                    runner_lines.append('      CMAKE_FLAGS="-DGGML_HIPBLAS=ON"')
                    runner_lines.append('    elif [ "$GPU_TYPE" = "SYCL" ]; then')
                    runner_lines.append('      CMAKE_FLAGS="-DGGML_SYCL=ON"')
                    runner_lines.append('    fi')
                    runner_lines.append('    cd ~/llama.cpp && rm -rf build && cmake -B build $CMAKE_FLAGS -DCMAKE_BUILD_TYPE=Release \\')
                    runner_lines.append('      && cmake --build build -j"$NPROC" --target llama-server \\')
                    runner_lines.append('      && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server')
                    runner_lines.append('  fi')
                    ...
                    runner_lines.append('    CMAKE_ARGS=""')
                    runner_lines.append('    if [ "$GPU_TYPE" = "CUDA" ]; then')
                    runner_lines.append('      CMAKE_ARGS="-DGGML_CUDA=ON"')
                    runner_lines.append('    elif [ "$GPU_TYPE" = "ROCm" ]; then')
                    runner_lines.append('      CMAKE_ARGS="-DGGML_HIPBLAS=ON"')
                    runner_lines.append('      export ROCm_PATH="/opt/rocm"')
                    runner_lines.append('    elif [ "$GPU_TYPE" = "SYCL" ]; then')
                    runner_lines.append('      CMAKE_ARGS="-DGGML_SYCL=ON"')
                    runner_lines.append('    fi')
                    runner_lines.append('    CMAKE_ARGS="$CMAKE_ARGS" pip install --user --break-system-packages -q \'llama-cpp-python[server]\' 2>/dev/null || CMAKE_ARGS="$CMAKE_ARGS" pip install -q \'llama-cpp-python[server]\' 2>/dev/null || true')
                    runner_lines.append('  fi')
                    runner_lines.append('  if ! command -v llama-server &>/dev/null && ! python3 -c "import llama_cpp" 2>/dev/null; then')
