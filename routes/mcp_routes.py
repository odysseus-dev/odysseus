...
def _mcp_oauth_base_dir() -> Path:
    """Directory that may contain OAuth files managed by Void."""
    ...
    async def oauth_callback(code: str, state: str, request: Request):
        ...
        if resolve_pending(state, code):
            return HTMLResponse(_oauth_result_page(
                ...
                "The MCP server is connecting. You can close this window and return to Void.",
                success=True,
            ))
...
<html><head>
<meta charset="UTF-8"><title>Authorize — Void</title>
<style>
