...
async def _eval_skill_run(
    ...
    def _parse(raw: str):
        ...
        if v not in _VERDICTS:
            km = _re.search(
                r'verdict["\'\s:]*\s*["\']?(pass|needs_work|fail|inconclusive)',
                text,
                _re.I,
            )
            if km:
...
async def _eval_skill_necessity(
    skill_md: str, others: list, url: str, model: str, headers: dict | None
) -> dict | None:
...
def _audit_generic_blocker(
    skill: dict | None, necessity: dict | None, verdict_data: dict | None
) -> str | None:
...
async def run_scheduled_skill_audit(
    skills_manager: SkillsManager, owner: str | None = None, max_skills: int = 8
) -> dict:
