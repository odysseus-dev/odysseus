...
async def _audit_one_skill(
    ...
    if (
        ...
        try:
            ...
            skills_manager.set_audit(
                name, "skipped", by_teacher=False, worker_model=model
            )
            if duplicate_of:
    ...
    if v in ("unknown", "inconclusive"):
        skills_manager.set_audit(
            name, "inconclusive", by_teacher=False, worker_model=model
        )
        status = _audit_finalize_status(
            skills_manager,
            name,
    ...
    if (
        ...
        if v == "pass":
            _set_conf(0.8)
            skills_manager.set_audit(
                name, "pass", by_teacher=True, worker_model=model, teacher_model=t_model
            )
            refreshed = next(
                (s for s in skills_manager.load(owner=owner) if s.get("name") == name),
                None,
