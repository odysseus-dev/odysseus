...
def _image_to_dict(img: GalleryImage, session_name: str = None) -> Dict[str, Any]:
    ...
    loras = None
    if getattr(img, "loras_json", None):
        try:
            loras = _json.loads(img.loras_json)
        except Exception:
            loras = img.loras_json
    reference_images = None
    if getattr(img, "reference_images_json", None):
        try:
            reference_images = _json.loads(img.reference_images_json)
        except Exception:
            reference_images = img.reference_images_json
    return {
        "id": img.id,
        ...
        "size": img.size,
        "quality": img.quality,
        "gen_seed": getattr(img, "gen_seed", None),
        ...
        "scheduler": getattr(img, "scheduler_name", None),
        "clip_skip": getattr(img, "clip_skip", None),
        "gen_style": getattr(img, "gen_style", None),
        "gen_op": getattr(img, "gen_op", None),
        "source_image_id": getattr(img, "source_image_id", None),
        "loras": loras,
        "reference_images": reference_images,
        "ip_method": getattr(img, "ip_method", None),
        "ip_weight": getattr(img, "ip_weight", None),
        "tags": img.tags or "",
