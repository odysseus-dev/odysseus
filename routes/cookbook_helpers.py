...
def _append_llama_cpp_linux_accel_build_lines(runner_lines: list[str]) -> None:
    ...
    runner_lines.append('    case "$_odysseus_force_cuda" in ON|1|TRUE) _odysseus_cuda_requested=1 ;; esac')
    ...
    runner_lines.append('      else')
    runner_lines.append('        if [ "$_odysseus_cuda_requested" -eq 1 ]; then')
    runner_lines.append('          echo "[odysseus] ERROR: ODYSSEUS_LLAMA_CPP_CUDA=ON, but CUDA runtime (libcudart.so) is not visible."')
    runner_lines.append('          echo "[odysseus]   Rebuild the NVIDIA compose overlay image so CUDA build packages are installed."')
    ...
    runner_lines.append('    else')
    runner_lines.append('      if [ "$_odysseus_cuda_requested" -eq 1 ]; then')
    runner_lines.append('        echo "[odysseus] ERROR: ODYSSEUS_LLAMA_CPP_CUDA=ON, but nvcc is not visible."')
    runner_lines.append('        echo "[odysseus]   Rebuild the NVIDIA compose overlay image so CUDA build packages are installed."')
    runner_lines.append('        exit 1')
    runner_lines.append('      else')
