...
def _diagnose_serve_output(text: str) -> dict | None:
    ...
    The browser uses cookbook-diagnosis.js for clickable fixes. This gives the
    agent/tool path the same structured signal so it can retry with an adjusted
    command instead of guessing from raw tmux output.
    """
    if not text:
        return None
    tail = text[-6000:]
    ...
    cpu_kv_matches = re.findall(
        r"CPU\s+KV\s+buffer\s+size\s*=\s*([\d.]+)\s*(MiB|GiB)",
        tail,
        flags=re.I,
    )
    if cpu_kv_matches:
        max_cpu_kv_mib = max(
            float(value) * (1024 if unit.lower() == "gib" else 1)
            for value, unit in cpu_kv_matches
        )
        if max_cpu_kv_mib >= 8192:
            return {
                "message": "llama.cpp allocated a large KV cache in system RAM.",
                "suggestions": [
                    {"label": "edit serve: choose q8_0/q4_0 KV cache or lower context", "op": "manual"},
                    {"label": "retry with context 8192", "op": "replace", "flag": "-c", "value": "8192"},
                ],
            }
