...
def _local_tooling_path_export(executable: str) -> str:
    ...
    When Vita Machina runs from a virtualenv, that bin dir holds the tools the
...
def _venv_safe_local_pip_install_cmd(cmd: str, *, local: bool, in_venv: bool) -> str:
    ...
    prepends Vita Machina's own interpreter directory to PATH. If Vita Machina itself is
...
def _ollama_bind_from_cmd(cmd: str | None, *, default_host: str = "127.0.0.1") -> tuple[str, str]:
    ...
    wider default host so the resulting API is reachable by Vita Machina.
    """
    if not cmd:
...
def _append_vllm_linux_preflight_lines(runner_lines: list[str]) -> None:
    ...
    # Keep the user install bin visible for Vita Machina-managed `pip install --user`
