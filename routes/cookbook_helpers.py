...
def _append_llama_cpp_linux_accel_build_lines(runner_lines: list[str]) -> None:
    ...
    runner_lines.append('        _odysseus_cuda_cmake_flags=()')
    runner_lines.append('        if ! [ -f "${CUDA_HOME:-/usr/local/cuda}/include/cub/cub.cuh" ]; then')
    runner_lines.append('          _odysseus_cuda_cccl_paths="${CUDA_HOME:-/usr/local/cuda}/include/cccl ~/.local/lib/python*/site-packages/nvidia/cu*/include/cccl ~/.local/lib/python*/dist-packages/nvidia/cu*/include/cccl /usr/local/lib/python*/site-packages/nvidia/cu*/include/cccl /usr/local/lib/python*/dist-packages/nvidia/cu*/include/cccl /usr/lib/python*/site-packages/nvidia/cu*/include/cccl /usr/lib/python*/dist-packages/nvidia/cu*/include/cccl /usr/include/cccl /usr/local/include/cccl"')
    runner_lines.append('          for _cccl in $_odysseus_cuda_cccl_paths; do')
    runner_lines.append('            [ -f "$_cccl/cub/cub.cuh" ] || continue')
    runner_lines.append('            _odysseus_cuda_cmake_flags+=("-DCMAKE_CUDA_FLAGS=${CMAKE_CUDA_FLAGS:-}${CMAKE_CUDA_FLAGS:+ }-I$_cccl")')
    runner_lines.append('            echo "[odysseus] CUDA CCCL headers found at $_cccl"')
    runner_lines.append('            break')
    runner_lines.append('          done')
    runner_lines.append('        fi')
    runner_lines.append('        cmake -B build -DCMAKE_BUILD_TYPE=Release -DGGML_CUDA=ON "${_odysseus_cuda_cmake_flags[@]}" && cmake --build build -j"$NPROC" --target llama-server && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server')
