...
def _hf_download_dir_pair(local_dir: str) -> tuple[str, str]:
    """Return (HF_HOME, HUGGINGFACE_HUB_CACHE) path strings for a download dir."""
    v = local_dir.rstrip("/\\")
    norm = v.replace("\\", "/")
    if norm.endswith("/hub"):
        parent = norm[:-4]
        return (parent if parent else "~"), v
    return v, f"{v}/hub"


def _hf_download_cache_env(local_dir: str) -> tuple[str, str]:
    """Return shell-quoted (HF_HOME, HUGGINGFACE_HUB_CACHE) for a download dir.

    Custom dirs use HF_HOME=<dir> and cache under <dir>/hub. When the user
    picks the default leaf ``~/.cache/huggingface/hub``, HF_HOME must be the
    parent (``~/.cache/huggingface``) — otherwise blobs land in hub/hub/ where
    the cache scanner never looks.
    """
    hf_home, hub = _hf_download_dir_pair(local_dir)
    return _shell_path(hf_home), _shell_path(hub)


def _ps_path_expr(p: str) -> str:
    """PowerShell expression for a validated path (tilde → $HOME)."""
    if p == "~":
        return "$HOME"
    if p.startswith("~/"):
        return f"Join-Path $HOME '{_ps_squote(p[2:])}'"
    return f"'{_ps_squote(p)}'"
...
def _append_llama_server_python_shim_lines(
    runner_lines: list[str],
    *,
    python_cmd: str = "python3",
    indent: str = "  ",
) -> None:
    """Append bash that creates ~/bin/llama-server when only pip bindings exist."""
    ind = indent
    py = python_cmd
    runner_lines.append(
        f'{ind}if ! command -v llama-server >/dev/null 2>&1 && {py} -c "import llama_cpp" 2>/dev/null; then'
    )
    runner_lines.append(f"{ind}  mkdir -p ~/bin")
    runner_lines.append(f"{ind}  cat > ~/bin/llama-server <<'_ODY_LLAMA_SHIM_EOF'")
    ...
    runner_lines.append('    -ub|--ubatch-size) shift 2 ;;')
    ...
    runner_lines.append('    --image-max-tokens) shift 2 ;;')
    runner_lines.append('    --no-mmap) ARGS+=(--no_mmap true); shift ;;')
    runner_lines.append('    --no-warmup) shift ;;')
    runner_lines.append('    --chat-template) ARGS+=(--chat_format "$2"); shift 2 ;;')
    runner_lines.append('    --fit|--split-mode|--tensor-split|--main-gpu|--parallel) shift 2 ;;')
    runner_lines.append('    --mlock) ARGS+=(--use_mlock true); shift ;;')
    runner_lines.append('    *) ARGS+=("$1"); shift ;;')
    runner_lines.append('  esac')
    runner_lines.append('done')
    runner_lines.append(f'exec {py} -m llama_cpp.server "${{ARGS[@]}}"')
    runner_lines.append("_ODY_LLAMA_SHIM_EOF")
    runner_lines.append(f"{ind}  chmod +x ~/bin/llama-server")
    runner_lines.append(
        f'{ind}  echo "[odysseus] Created llama-server shim → {py} -m llama_cpp.server (no native binary needed)"'
    )
    runner_lines.append(f"{ind}fi")
