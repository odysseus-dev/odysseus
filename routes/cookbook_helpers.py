...
def _cached_model_scan_script(model_dirs: list[str] | None = None, add_hf_cache: str | None = None) -> str:
    ...
    lines = [
        ...
        "    if hf_home: add(os.path.join(hf_home, 'hub'))",
                "    # Best-effort: ask huggingface_hub for its own resolved cache.",
        "    # Its defaults are platform-aware - macOS uses ~/Library/Caches/huggingface",
        "    # (NOT ~/.cache/...), which is what issue #5978 reported as missing. Falls",
        "    # back silently so the scanner still runs on remote hosts without the lib.",
        "    try:",
        "        import huggingface_hub",
        "        _c = getattr(huggingface_hub, 'constants', None)",
        "        _hc = getattr(_c, 'HF_HUB_CACHE', None) or getattr(_c, 'hf_hub_cache', None)",
        "        if _hc: add(_hc)",
        "    except Exception:",
        "        pass",
        "    # Platform-aware hard-coded fallback (covers hosts lacking huggingface_hub",
        "    # so the cache is still discoverable on macOS even without the library).",
