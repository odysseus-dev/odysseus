...
def _append_llama_cpp_source_bootstrap_lines(runner_lines: list[str]) -> None:
    """Append shell lines that ensure ~/llama.cpp is a source checkout."""
    runner_lines.append('  _odysseus_ensure_llama_cpp_source() {')
    runner_lines.append('    local _dst="$HOME/llama.cpp"')
    runner_lines.append('    [ -f "$_dst/CMakeLists.txt" ] && return 0')
    runner_lines.append('    if [ -e "$_dst" ] && [ ! -d "$_dst" ]; then')
    runner_lines.append('      echo "ERROR: $_dst exists but is not a directory; move it aside and relaunch."')
    runner_lines.append('      return 1')
    runner_lines.append('    fi')
    runner_lines.append('    if [ -d "$_dst" ]; then')
    runner_lines.append('      if ! find "$_dst" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null | grep -q .; then')
    runner_lines.append('        git clone --depth 1 https://github.com/ggml-org/llama.cpp "$_dst"')
    runner_lines.append('        return $?')
    runner_lines.append('      fi')
    runner_lines.append('      if find "$_dst" -mindepth 1 -maxdepth 1 ! -name .odysseus -print -quit 2>/dev/null | grep -q .; then')
    runner_lines.append('        echo "ERROR: $_dst exists but is not a llama.cpp source checkout (missing CMakeLists.txt)."')
    runner_lines.append('        echo "Move that directory aside or remove non-Odysseus files, then relaunch."')
    runner_lines.append('        echo "Top-level entries in $_dst:"')
    runner_lines.append('        local _entry')
    runner_lines.append('        for _entry in "$_dst"/* "$_dst"/.[!.]* "$_dst"/..?*; do')
    runner_lines.append('          [ -e "$_entry" ] || continue')
    runner_lines.append('          printf "  %s\\n" "$(basename "$_entry")"')
    runner_lines.append('        done')
    runner_lines.append('        return 1')
    runner_lines.append('      fi')
    runner_lines.append('      local _tmp')
    runner_lines.append('      _tmp="$(mktemp -d "${TMPDIR:-/tmp}/odysseus-llama-cpp.XXXXXX")" || return 1')
    runner_lines.append('      if git clone --depth 1 https://github.com/ggml-org/llama.cpp "$_tmp/source"; then')
    runner_lines.append('        if cp -a "$_tmp/source"/. "$_dst"/; then')
    runner_lines.append('          rm -rf "$_tmp"')
