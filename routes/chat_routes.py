...
def setup_chat_routes(
    ...
    async def chat_endpoint(request: Request, chat_request: ChatRequest) -> Dict[str, Any]:
        ...
        ctx = await build_chat_context(
            ...
            no_think=bool(chat_request.no_think),
        )
    ...
    async def chat_stream(request: Request) -> StreamingResponse:
        ...
        thinking_raw = form_data.get("thinking") if form_data.get("thinking") is not None else (body or {}).get("thinking")
        no_think_raw = form_data.get("no_think") if form_data.get("no_think") is not None else (body or {}).get("no_think")
        thinking = None
        if thinking_raw is not None:
            thinking = str(thinking_raw).lower() not in {"false", "0", "no"}
        no_think = False
        if no_think_raw is not None:
