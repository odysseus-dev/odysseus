...
def setup_email_routes():
    ...
    def _list_emails_sync(
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                status, data = conn.search(
                    None, f'(UNANSWERED SINCE "{_since}"{from_clause})'
                )
                ...
                status, data = conn.search(
                    None, f'(UNANSWERED BEFORE "{_before}"{from_clause})'
                )
            ...
            if uid_list:
                ...
                try:
                    status, msg_data = conn.fetch(
                        fetch_set, "(FLAGS RFC822.HEADER RFC822.SIZE)"
                    )
                except Exception as e:
                    ...
                    try:
                        ...
                        for part in msg_data:
                            if isinstance(part, tuple):
                                ...
                                if (
                                    b"RFC822.HEADER" in part[0]
                                    if isinstance(part[0], bytes)
                                    else "RFC822.HEADER" in meta
                                ):
                                    raw_header = part[1]
    ...
    async def read_email_by_uid(
        ...
        if cached is not None:
            return cached
        result = await _asyncio.to_thread(
            _read_email_sync, uid, folder, account_id, owner
        )
        if result and not result.get("error"):
    ...
    async def compose_upload(
        ...
        try:
            ...
            if len(content) > MAX_BYTES:
                raise HTTPException(
                    413, f"Attachment exceeds {MAX_BYTES // (1024 * 1024)}MB limit"
                )
    ...
    async def update_email_config(data: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            if row is None:
                row = EmailAccount(
                    id=_uuid.uuid4().hex, name="Default", is_default=True, enabled=True
                )
                db.add(row)
            field_map = {
                "smtp_host": "smtp_host",
