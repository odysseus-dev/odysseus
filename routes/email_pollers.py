                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Void (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Void" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "void_kind" if "void_kind" in cols else "'scheduled' AS void_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                void_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Void-Origin"] = "void-ui"
                outer["X-Void-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", void_kind or "scheduled")[:64]
                outer["X-Void-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `void-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `VOID_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `void-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("VOID_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `VOID_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (VOID_INPROCESS_POLLERS=0); "
            "drive `void-mail poll-scheduled` externally."
        )
        return
    import asyncio
