                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Juniperus (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Juniperus" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "juniperus_kind" if "juniperus_kind" in cols else "'scheduled' AS juniperus_kind"
        rows = conn.execute(f"""
        ...
        for r in rows:
            ...
            try:
                ...
                juniperus_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Juniperus-Origin"] = "juniperus-ui"
                outer["X-Juniperus-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", juniperus_kind or "scheduled")[:64]
                outer["X-Juniperus-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `juniperus-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `JUNIPERUS_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `juniperus-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("JUNIPERUS_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `JUNIPERUS_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (JUNIPERUS_INPROCESS_POLLERS=0); "
            "drive `juniperus-mail poll-scheduled` externally."
        )
        return
    import asyncio
