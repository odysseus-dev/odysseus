...
async def _auto_summarize_pass(
    days_back: int = 1, account_id: str | None = None
) -> str:
    ...
    if account_id is None:
        ...
        if len(ids) <= 1:
            ...
            return await _auto_summarize_pass_single(
                days_back=days_back, account_id=(ids[0] if ids else None)
            )
        outs = []
        for aid in ids:
            try:
                result = await _auto_summarize_pass_single(
                    days_back=days_back, account_id=aid
                )
...
async def _auto_summarize_pass_single(
    ...
    try:
        ...
        _sum_existing = {
            r[0]
            for r in _c.execute("SELECT message_id FROM email_summaries").fetchall()
        }
        _reply_existing = {
            r[0]
            for r in _c.execute("SELECT message_id FROM email_ai_replies").fetchall()
        }
        _tag_existing = (
            {r[0] for r in _c.execute("SELECT message_id FROM email_tags").fetchall()}
        ...
        _urgent_existing = (
            {
                r[0]
                for r in _c.execute(
                    "SELECT message_id FROM email_urgency_alerts"
                ).fetchall()
            }
            if auto_urgent
            else set()
        ...
        try:
            _self_self_addr = (
                (_get_email_config(account_id).get("from_address") or "")
                .strip()
                .lower()
            )
        except Exception:
            ...
            try:
                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    open_url = (
                                        f"{_pub}/#email={_q(_folder, safe='')}:{uid_str}"
                                        if _pub
                                        else ""
                                    )
                ...
                if need_class:
                    try:
                        ...
                        if not resp.ok:
                            logger.warning(
                                f"Auto-classify {uid.decode()} HTTP {resp.status_code}: {resp.text[:200]}"
                            )
                        else:
                            rdata = resp.json()
                            m = (rdata.get("choices") or [{}])[0].get("message", {})
                            ...
                            if parsed is not None:
                                ...
                                if is_spam and auto_spam and spam_folder:
                                    if _imap_move(uid, spam_folder):
                                        ...
                                        logger.info(
                                            f"Auto-spam moved uid={uid.decode()} to {spam_folder}: {spam_reason}"
                                        )
