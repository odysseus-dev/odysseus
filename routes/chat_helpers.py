...
def resolve_session_auth(sess, session_id: str):
    ...
    has_auth = (
        sess.headers
        and isinstance(sess.headers, dict)
        and any(k.lower() in ("authorization", "x-api-key") for k in sess.headers)
    )
    if has_auth:
        return
    ...
    try:
        ...
        try:
            domain = (
                sess.endpoint_url.split("//")[1].split("/")[0]
                if "//" in sess.endpoint_url
                else ""
            )
            if domain:
                ep = (
                    db.query(ModelEndpoint)
                    .filter(ModelEndpoint.base_url.contains(domain))
                    .first()
                )
