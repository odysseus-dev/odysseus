...
def setup_gallery_routes() -> APIRouter:
        ...
        try:
            ...
            for a in albums:
                ...
                if a.cover_id:
                    cover = (
                        db.query(GalleryImage)
                        .filter(GalleryImage.id == a.cover_id)
                        .first()
                    )
                    if cover:
        ...
        try:
            ...
            with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
                for img in imgs:
                    ...
                    base = (img.prompt or "").strip() or os.path.splitext(img.filename)[
                        0
                    ]
    ...
    async def clear_gallery_ai_tags(
        request: Request, image_id: str | None = Query(None)
    ) -> dict[str, Any]:
        user = get_current_user(request)
        db = SessionLocal()
        try:
    ...
    async def harmonize_image(request: Request):
        ...
        if not base:
            ...
            try:
                for ep in db.query(ModelEndpoint).all():
                    if ep.base_url.rstrip("/").removesuffix("/v1").rstrip(
                        "/"
                            ...
                            if item.get("url"):
                                async with httpx.AsyncClient(timeout=60) as c2:
                                    ...
                                    if ir.status_code == 200:
                                        return {
                                            "image": _b64.b64encode(ir.content).decode()
                                        }
