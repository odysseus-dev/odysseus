...
def _explicit_model_list_timeout(base_url: str, endpoint_kind: str = "auto", requested: Any = None) -> float:
    ...
    # Use _get_probe_timeout for user-facing probes
    return _get_probe_timeout(category)
...
def _get_probe_timeout(category: str) -> float:
...
def setup_model_routes(model_discovery):
    ...
    def ping_endpoints(request: Request):
        ...
        for ep in endpoints:
            ...
            try:
                t0 = _time.time()
                category = _classify_endpoint(base, kind)
                ping_timeout = _get_probe_timeout(category)
                ping = _ping_endpoint(base, ep.api_key, timeout=ping_timeout)
    ...
    def probe_endpoint_models(ep_id: str, request: Request):
        ...
        base = _normalize_base(ep_data["base_url"])
        category = _classify_endpoint(base, "auto")
        probe_timeout = _get_probe_timeout(category)
        all_models = _probe_endpoint(base, ep_data["api_key"], timeout=probe_timeout)
