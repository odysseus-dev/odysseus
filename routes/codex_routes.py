...
def setup_hermes_routes() -> APIRouter:
    """Serve the Hermes Agent skill bundle.

    Hermes uses the same scope-gated `/api/codex/*` endpoints at runtime;
    this router only exists to deliver the skill zip via `/api/hermes/plugin.zip`
    so the user-facing setup commands stay in the Hermes namespace.
    """
    ...
    def plugin_zip(request: Request):
        require_authenticated_request(request)
        # Only ship the skills/ subtree so extracting at ~/.hermes/ doesn't dump
        # README.md or other bundle metadata into the user's hermes config dir.
        skills_root = Path(__file__).resolve().parent.parent / "integrations" / "hermes" / "skills"
        if not skills_root.exists():
            raise HTTPException(404, "Hermes skill bundle not found")
        bundle_root = skills_root.parent
        buf = BytesIO()
        with zipfile.ZipFile(buf, "w", compression=zipfile.ZIP_DEFLATED) as zf:
            for path in sorted(skills_root.rglob("*")):
                ...
                zf.write(path, path.relative_to(bundle_root))
        buf.seek(0)
        headers = {"Content-Disposition": 'attachment; filename="odysseus-hermes-skill.zip"'}
        return StreamingResponse(buf, media_type="application/zip", headers=headers)
