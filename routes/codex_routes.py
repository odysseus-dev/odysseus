...
def setup_agy_routes() -> APIRouter:
    """Serve the Antigravity skill bundle.

    Antigravity uses the same scope-gated `/api/codex/*` endpoints at runtime;
    this router only exists to deliver the skill zip via `/api/agy/plugin.zip`
    so the user-facing setup commands stay in the Antigravity namespace.
    """
    ...
    def plugin_zip(request: Request):
        require_authenticated_request(request)
        # Only ship the skills/ subtree so extracting at ~/.gemini/config/ doesn't dump
        # README.md or other bundle metadata into the user's config dir.
        skills_root = Path(__file__).resolve().parent.parent / "integrations" / "agy" / "skills"
        if not skills_root.exists():
            raise HTTPException(404, "Antigravity skill bundle not found")
        buf = BytesIO()
        with zipfile.ZipFile(buf, "w", compression=zipfile.ZIP_DEFLATED) as zf:
            for path in sorted(skills_root.rglob("*")):
                if path.is_dir() or "__pycache__" in path.parts or path.suffix == ".pyc":
                    continue
                zf.write(path, path.relative_to(skills_root))
        buf.seek(0)
