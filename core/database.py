...
class ChatMessage(Base):
    ...
    session_id = Column(String, ForeignKey(
        "sessions.id", ondelete="CASCADE"), nullable=False, index=True)

    # Message content
    role = Column(String, nullable=False)
    content = Column(Text, nullable=False)
    # JSON string for metrics etc.
...
class Document(TimestampMixin, Base):
    ...
    id = Column(String, primary_key=True, index=True)
    session_id = Column(String, ForeignKey(
        "sessions.id", ondelete="SET NULL"), nullable=True, index=True)
    title = Column(String, nullable=False, default="Untitled")
    # "python", "markdown", "text", etc.
    ...
    owner = Column(String, nullable=True, index=True)
    # "keep", "junk", or None (not yet reviewed)
    ...
    session = relationship("Session", backref=backref(
        "documents", cascade="save-update, merge"))
...
class GalleryImage(TimestampMixin, Base):
    ...
    session_id = Column(String, ForeignKey(
        "sessions.id", ondelete="SET NULL"), nullable=True, index=True)
    album_id = Column(String, ForeignKey("gallery_albums.id",
...
class EmailAccount(TimestampMixin, Base):
    ...
    id = Column(String, primary_key=True, index=True)
    owner = Column(String, nullable=True, index=True)
    # Display name: "Work", "Personal", etc.
    name = Column(String, nullable=False)
...
class ModelEndpoint(TimestampMixin, Base):
    ...
    id = Column(String, primary_key=True, index=True)
    # Display label, e.g. "Local vLLM", "OpenRouter"
    name = Column(String, nullable=False)
    # Base URL, e.g. "http://localhost:8002/v1"
    base_url = Column(String, nullable=False)
    # Optional provider API key, encrypted at rest
    api_key = Column(EncryptedText, nullable=True)
    is_enabled = Column(Boolean, default=True)
    # JSON list of model IDs that failed probing
...
class McpServer(TimestampMixin, Base):
    ...
    is_enabled = Column(Boolean, default=True)
    # JSON: provider, keys_file, token_file, scopes
    oauth_config = Column(Text, nullable=True)
    # JSON array of tool names to hide from LLM
...
class Comparison(TimestampMixin, Base):
    ...
    id = Column(String, primary_key=True, index=True)
    # Parent session context (optional)
...
class Signature(TimestampMixin, Base):
    ...
    # base64 PNG, encrypted at rest
    data_png = Column(EncryptedText, nullable=False)
    width = Column(Integer, nullable=True)
    height = Column(Integer, nullable=True)
    # vector signature, encrypted at rest
...
class UserTool(TimestampMixin, Base):
    ...
    session = relationship("Session", backref=backref(
        "user_tools", cascade="all, delete-orphan"))
...
class UserToolData(Base):
    ...
    tool_id = Column(String, ForeignKey(
        "user_tools.id", ondelete="CASCADE"), nullable=False)
    key = Column(String, nullable=False)
    value = Column(Text, nullable=True)
    created_at = Column(DateTime, default=utcnow_naive)
    updated_at = Column(DateTime, default=utcnow_naive, onupdate=utcnow_naive)

    tool = relationship("UserTool", backref=backref(
        "data_entries", cascade="all, delete-orphan"))
...
class ScheduledTask(TimestampMixin, Base):
    ...
    # max agent loop iterations (null=unlimited)
    max_steps = Column(Integer, nullable=True)
    # email results to character.email_to
    email_results = Column(Boolean, default=True)
    # per-task on/off for completion notifications
    notifications_enabled = Column(Boolean, default=True)

    session = relationship("Session", backref=backref(
        "scheduled_tasks", cascade="save-update, merge"))
    then_task = relationship("ScheduledTask", remote_side=[
                             id], foreign_keys=[then_task_id])

    __table_args__ = (
        Index('ix_scheduled_tasks_due', 'status', 'next_run'),
        Index('ix_scheduled_tasks_event',
...
class EditorDraft(TimestampMixin, Base):
    ...
    __table_args__ = (
        Index('ix_editor_drafts_owner_updated',
...
class TaskRun(Base):
    ...
    id = Column(String, primary_key=True, index=True)
    task_id = Column(String, ForeignKey("scheduled_tasks.id",
    ...
    # JSON log of agent tool calls
    steps = Column(Text, nullable=True)
    # model that actually ran (resolved at execution)
    model = Column(String, nullable=True)
...
def _migrate_add_last_message_at_column():
    ...
    try:
        ...
        logging.getLogger(__name__).warning(
            f"last_message_at migration failed: {e}")
...
def _migrate_add_document_archived_column():
    ...
    try:
        ...
        if "archived" not in columns:
            conn.execute(
                "ALTER TABLE documents ADD COLUMN archived BOOLEAN DEFAULT 0")
            conn.commit()
            logging.getLogger(__name__).info(
                "Migrated: added 'archived' to documents")
        conn.close()
    except Exception as e:
        logging.getLogger(__name__).warning(
            f"documents.archived migration failed: {e}")
...
def _migrate_add_owner_column():
    ...
    try:
        ...
        if "owner" not in columns:
            ...
            conn.commit()
            logging.getLogger(__name__).info(
                "Migrated: added 'owner' column to sessions")
        conn.close()
    except Exception as e:
...
def _migrate_model_endpoints():
    ...
    try:
        ...
        logging.getLogger(__name__).warning(
            f"model_endpoints migration check failed: {e}")
...
def _migrate_add_hidden_models_column():
    ...
    try:
        ...
        if columns and "hidden_models" not in columns:
            conn.execute(
                "ALTER TABLE model_endpoints ADD COLUMN hidden_models TEXT")
            conn.commit()
            logging.getLogger(__name__).info(
        ...
        logging.getLogger(__name__).warning(
            f"hidden_models migration failed: {e}")
...
def _migrate_add_model_type_column():
    ...
    try:
        ...
        if columns and "model_type" not in columns:
            conn.execute(
                "ALTER TABLE model_endpoints ADD COLUMN model_type TEXT DEFAULT 'llm'")
            conn.commit()
            logging.getLogger(__name__).info(
        ...
        logging.getLogger(__name__).warning(
            f"model_type migration failed: {e}")
...
def _migrate_add_model_endpoint_refresh_columns():
    ...
    try:
        ...
        if columns and "endpoint_kind" not in columns:
            conn.execute(
                "ALTER TABLE model_endpoints ADD COLUMN endpoint_kind TEXT DEFAULT 'auto'")
        ...
        if columns and "model_refresh_interval" not in columns:
            conn.execute(
                "ALTER TABLE model_endpoints ADD COLUMN model_refresh_interval INTEGER")
        if columns and "model_refresh_timeout" not in columns:
            conn.execute(
                "ALTER TABLE model_endpoints ADD COLUMN model_refresh_timeout INTEGER")
        conn.commit()
        conn.close()
    except Exception as e:
        logging.getLogger(__name__).warning(
            f"model_endpoints refresh-policy migration failed: {e}")
...
def _migrate_add_task_run_model_column():
    ...
    try:
        ...
        if columns and "model" not in columns:
            ...
            conn.commit()
            logging.getLogger(__name__).info(
                "Migrated: added 'model' column to task_runs")
        conn.close()
    except Exception as e:
        logging.getLogger(__name__).warning(
            f"task_runs model migration failed: {e}")
...
def _migrate_add_supports_tools_column():
    ...
    try:
        ...
        if columns and "supports_tools" not in columns:
            conn.execute(
                "ALTER TABLE model_endpoints ADD COLUMN supports_tools BOOLEAN")
            conn.commit()
            logging.getLogger(__name__).info(
        ...
        logging.getLogger(__name__).warning(
            f"supports_tools migration failed: {e}")
...
def _migrate_add_cached_models_column():
    ...
    try:
        ...
        if columns and "cached_models" not in columns:
            conn.execute(
                "ALTER TABLE model_endpoints ADD COLUMN cached_models TEXT")
            conn.commit()
        conn.close()
    except Exception as e:
        logging.getLogger(__name__).warning(
            f"cached_models migration failed: {e}")
...
def _migrate_add_pinned_models_column():
    ...
    try:
        ...
        if columns and "pinned_models" not in columns:
            conn.execute(
                "ALTER TABLE model_endpoints ADD COLUMN pinned_models TEXT")
            conn.commit()
            logging.getLogger(__name__).info(
        ...
        logging.getLogger(__name__).warning(
            f"pinned_models migration failed: {e}")
...
def _migrate_add_notes_sort_order():
    ...
    try:
        ...
        if columns and "repeat" not in columns:
            conn.execute(
                "ALTER TABLE notes ADD COLUMN repeat TEXT DEFAULT 'none'")
...
def _migrate_add_mode_column():
    ...
    try:
        ...
        logging.getLogger(__name__).warning(
            f"Migration check for mode failed: {e}")
...
def _migrate_add_folder_column():
    ...
    try:
        ...
        logging.getLogger(__name__).warning(
            f"Migration check for folder failed: {e}")
...
def _migrate_add_token_columns():
    ...
    try:
        ...
        if "total_input_tokens" not in columns:
            ...
            conn.commit()
            logging.getLogger(__name__).info(
                "Migrated: added token tracking columns to sessions")
        conn.close()
    except Exception as e:
        logging.getLogger(__name__).warning(
            f"Migration check for token columns failed: {e}")
...
def _migrate_add_api_token_scopes_column():
    ...
    try:
        ...
        columns = [row[1] for row in conn.execute(
            "PRAGMA table_info(api_tokens)").fetchall()]
        if columns and "scopes" not in columns:
            ...
            conn.commit()
            logging.getLogger(__name__).info(
                "Migrated: added scopes column to api_tokens")
        conn.close()
    except Exception as e:
        logging.getLogger(__name__).warning(
            f"api_tokens.scopes migration failed: {e}")
...
def _migrate_assign_legacy_owner():
    ...
    auth_path = os.path.join(os.path.dirname(
        DATABASE_URL.replace("sqlite:///", "")), "auth.json")
    ...
    try:
        ...
        for table in tables:
            try:
                ...
                logger.warning(
                    f"Legacy owner assignment for {table} failed: {e}")
        conn.commit()
        conn.close()
    except Exception as e:
    ...
    try:
        if os.path.exists(prefs_path):
            ...
            if "_users" not in prefs and prefs:
                ...
                logger.info(
                    f"Migrated user_prefs.json to per-user format under '{admin_user}'")
    except Exception as e:
...
def _migrate_backfill_document_owner_from_session():
    ...
    try:
        with engine.connect() as conn:
            cols = [r[1]
                    for r in conn.execute(text("PRAGMA table_info(documents)"))]
    ...
    try:
        with engine.connect() as conn:
            ...
            if "tidy_verdict" not in cols:
                conn.execute(
                    text("ALTER TABLE documents ADD COLUMN tidy_verdict VARCHAR"))
                conn.commit()
                logging.getLogger(__name__).info(
                    "Added tidy_verdict column to documents")
    except Exception as e:
        ...
        with engine.connect() as conn:
            ...
            for col, spec in cols_to_add.items():
                if col not in existing:
                    conn.execute(
                        text(f"ALTER TABLE documents ADD COLUMN {col} {spec}"))
                    logging.getLogger(__name__).info(
                        f"Added {col} column to documents")
...
def _migrate_add_task_automation_columns():
    ...
    try:
        with engine.connect() as conn:
            cols_info = list(conn.execute(
                text("PRAGMA table_info(scheduled_tasks)")))
            ...
            if needs_rebuild:
                ...
                conn.execute(
                    text("ALTER TABLE scheduled_tasks RENAME TO _old_scheduled_tasks"))
                conn.execute(text("""
    ...
    try:
        with engine.connect() as conn:
            ...
            if "oauth_config" not in cols:
                conn.execute(
                    text("ALTER TABLE mcp_servers ADD COLUMN oauth_config TEXT"))
                conn.commit()
                logging.getLogger(__name__).info(
    ...
    try:
        with engine.connect() as conn:
            ...
            if "disabled_tools" not in cols:
                conn.execute(
                    text("ALTER TABLE mcp_servers ADD COLUMN disabled_tools TEXT"))
                conn.commit()
                logging.getLogger(__name__).info(
    ...
    try:
        with engine.connect() as conn:
            ...
            if "oauth_tokens" not in cols:
                conn.execute(
                    text("ALTER TABLE mcp_servers ADD COLUMN oauth_tokens TEXT"))
                conn.commit()
                logging.getLogger(__name__).info(
                    "Added oauth_tokens column to mcp_servers")
    except Exception as e:
...
def _migrate_add_task_v2_columns():
    ...
    try:
        with engine.connect() as conn:
            ...
            conn.commit()
            logging.getLogger(__name__).info(
                "Task v2 columns migration complete")
    except Exception as e:
    ...
    try:
        with engine.connect() as conn:
            for action in targets:
                ...
                r = conn.execute(
                    text("DELETE FROM scheduled_tasks WHERE action=:a"), {"a": action})
                if r.rowcount:
                    logging.getLogger(__name__).info(
                        f"Dropped {r.rowcount} {action} task row(s)")
            conn.commit()
    except Exception as e:
...
def _migrate_add_notifications_enabled():
    ...
    try:
        ...
        logging.getLogger(__name__).warning(
            f"notifications_enabled migration: {e}")
...
def _migrate_add_crew_member_id():
    ...
    try:
        with engine.connect() as conn:
            cols = [r[1]
                    for r in conn.execute(text("PRAGMA table_info(sessions)"))]
            if "crew_member_id" not in cols:
                conn.execute(
                    text("ALTER TABLE sessions ADD COLUMN crew_member_id TEXT"))
                conn.commit()
                logging.getLogger(__name__).info(
                    "Added crew_member_id column to sessions")
            cols2 = [r[1] for r in conn.execute(
                text("PRAGMA table_info(scheduled_tasks)"))]
            if "crew_member_id" not in cols2:
                conn.execute(
                    text("ALTER TABLE scheduled_tasks ADD COLUMN crew_member_id TEXT"))
                conn.commit()
                logging.getLogger(__name__).info(
                    "Added crew_member_id column to scheduled_tasks")
    except Exception as e:
...
def _migrate_add_assistant_columns():
    ...
    try:
        with engine.connect() as conn:
            cols = [r[1] for r in conn.execute(
                text("PRAGMA table_info(crew_members)"))]
            ...
            if "timezone" not in cols:
                conn.execute(
                    text("ALTER TABLE crew_members ADD COLUMN timezone TEXT"))
                conn.commit()
                logging.getLogger(__name__).info(
                    "Added timezone column to crew_members")
    except Exception as e:
        logging.getLogger(__name__).warning(
            f"assistant columns migration: {e}")
...
class Note(TimestampMixin, Base):
    ...
    session_id = Column(String, nullable=True)
    sort_order = Column(Integer, default=0)
    # uploaded image URL (relative path)
...
class CalendarCal(TimestampMixin, Base):
    ...
    # Optional ICS subscription URL. When set, "Refresh from database" re-fetches
...
class CalendarEvent(TimestampMixin, Base):
    ...
    calendar_id = Column(String, ForeignKey(
        "calendars.id"), nullable=False, index=True)
    ...
    # last time the assistant pinged about this event
    ...
    try:
        with engine.connect() as conn:
            ...
            existing = conn.execute(
                text("SELECT COUNT(*) FROM email_accounts")).scalar() or 0
            if existing > 0:
                return
        ...
        with engine.begin() as conn:
            ...
            logging.getLogger(__name__).info(
                "Seeded email_accounts 'Default' from settings.json")
    except Exception as e:
        logging.getLogger(__name__).warning(
            f"seed email account migration: {e}")
...
def _migrate_chat_messages_fts():
    ...
    try:
        ...
        try:
            conn.execute(
                "CREATE VIRTUAL TABLE IF NOT EXISTS temp._odysseus_fts5_probe USING fts5(content)")
            conn.execute("DROP TABLE IF EXISTS temp._odysseus_fts5_probe")
        except Exception as e:
            logging.getLogger(__name__).warning(
                f"chat_messages FTS migration skipped; FTS5 unavailable: {e}")
            return
        ...
        logging.getLogger(__name__).warning(
            f"chat_messages FTS migration failed: {e}")
    finally:
        try:
            conn.close()
        except Exception:
...
def _migrate_add_email_smtp_security():
    ...
    try:
        ...
        logging.getLogger(__name__).warning(
            f"smtp_security migration skipped: {e}")
...
def _migrate_encrypt_endpoint_keys():
    ...
    try:
        ...
        logger.warning(
            f"secret_storage import failed; skipping endpoint-key migration: {e}")
        return
    try:
        with engine.connect() as conn:
            rows = conn.execute(
                text("SELECT id, api_key FROM model_endpoints")).fetchall()
            migrated = 0
            for rid, key in rows:
            ...
            if migrated:
                conn.commit()
                logger.info(
                    f"Encrypted plaintext API key on {migrated} endpoint row(s)")
    except Exception as e:
...
def _migrate_encrypt_signatures():
    ...
    try:
        ...
        logger.warning(
            f"secret_storage import failed; skipping signature migration: {e}")
        return
    try:
        with engine.connect() as conn:
            ...
            for rid, data_png, svg in rows:
                ...
                if updates:
                    ...
                    conn.execute(text(f"UPDATE signatures SET {sets} WHERE id = :id"), {
                                 **updates, "id": rid})
                    migrated += 1
            if migrated:
                conn.commit()
                logger.info(
                    f"Encrypted plaintext signature(s) on {migrated} row(s)")
    except Exception as e:
    ...
    try:
        with engine.connect() as conn:
            ...
            if migrated:
                conn.commit()
                logger.info(
                    f"Encrypted plaintext passwords on {migrated} email account row(s)")
    except Exception as e:
        logger.warning(
            f"Password migration failed (will retry next start): {e}")
...
def _migrate_add_calendar_is_utc():
    ...
    try:
        ...
        if columns and "is_utc" not in columns:
            ...
            conn.commit()
            logging.getLogger(__name__).info(
                "Migrated: added 'is_utc' column to calendar_events")
        conn.close()
    except Exception as e:
...
def _migrate_add_calendar_origin():
    ...
    try:
        ...
        if columns and "origin" not in columns:
            ...
            conn.commit()
            logging.getLogger(__name__).info(
                "Migrated: added 'origin' column to calendar_events")
        conn.close()
    except Exception as e:
        logging.getLogger(__name__).warning(
            f"calendar_events.origin migration failed: {e}")
...
def _migrate_add_calendar_account_id():
    ...
    try:
        ...
        if columns and "account_id" not in columns:
            ...
            conn.commit()
            logging.getLogger(__name__).info(
                "Migrated: added 'account_id' column to calendars")
        conn.close()
    except Exception as e:
        logging.getLogger(__name__).warning(
            f"calendars.account_id migration failed: {e}")


def _migrate_add_calendar_ics_url():
    """Add `ics_url` to calendars so ICS-subscribed calendars can be refreshed
    by re-fetching the stored URL. Idempotent."""
    import sqlite3
    db_path = DATABASE_URL.replace("sqlite:///", "")
    if not os.path.exists(db_path):
        return
    try:
        ...
        cursor = conn.execute("PRAGMA table_info(calendars)")
        columns = [row[1] for row in cursor.fetchall()]
        if columns and "ics_url" not in columns:
            conn.execute("ALTER TABLE calendars ADD COLUMN ics_url TEXT")
            conn.commit()
            logging.getLogger(__name__).info(
                "Migrated: added 'ics_url' column to calendars")
        conn.close()
    except Exception as e:
        logging.getLogger(__name__).warning(
            f"calendars.ics_url migration failed: {e}")
...
def _migrate_add_calendar_metadata():
    ...
    try:
        ...
        if columns and "last_pinged" not in columns:
            conn.execute(
                "ALTER TABLE calendar_events ADD COLUMN last_pinged DATETIME")
        conn.commit()
        conn.close()
    except Exception as e:
        logging.getLogger(__name__).warning(
            f"calendar_events migration failed: {e}")
    ...
    try:
        with get_db_session() as db:
            db.query(Session).filter(Session.id ==
                                     session_id).update({"mode": mode})
        return True
    except Exception:
        logger.warning(
            "Failed to persist mode %r for session %s", mode, session_id)
        return False
