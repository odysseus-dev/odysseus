...
class Document(TimestampMixin, Base):
    ...
    session = relationship(
        "Session", backref=backref("documents", cascade="save-update, merge")
    )
...
class GalleryImage(TimestampMixin, Base):
    ...
    album_id = Column(
        String,
        ForeignKey("gallery_albums.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    owner = Column(String, nullable=True, index=True)
...
class UserTool(TimestampMixin, Base):
    ...
    session = relationship(
        "Session", backref=backref("user_tools", cascade="all, delete-orphan")
    )
...
class UserToolData(Base):
    ...
    tool_id = Column(
        String, ForeignKey("user_tools.id", ondelete="CASCADE"), nullable=False
    )
    ...
    tool = relationship(
        "UserTool", backref=backref("data_entries", cascade="all, delete-orphan")
    )
...
class ScheduledTask(TimestampMixin, Base):
    ...
    then_task = relationship(
        "ScheduledTask", remote_side=[id], foreign_keys=[then_task_id]
    )
...
class TaskRun(Base):
    ...
    task = relationship(
        "ScheduledTask",
        backref=backref(
            "runs", cascade="all, delete-orphan", order_by="TaskRun.started_at.desc()"
        ),
    )
...
def _migrate_add_api_token_scopes_column():
    ...
    try:
        conn = sqlite3.connect(db_path)
        columns = [
            row[1] for row in conn.execute("PRAGMA table_info(api_tokens)").fetchall()
        ]
...
def _migrate_assign_legacy_owner():
    ...
    auth_path = os.path.join(
        os.path.dirname(DATABASE_URL.replace("sqlite:///", "")), "auth.json"
    )
...
def _migrate_add_doc_source_email_cols():
    ...
    try:
        with engine.connect() as conn:
            existing = {
                r[1] for r in conn.execute(text("PRAGMA table_info(documents)"))
            }
...
def _migrate_add_task_v2_columns():
    ...
    try:
        with engine.connect() as conn:
            cols = [
                r[1] for r in conn.execute(text("PRAGMA table_info(scheduled_tasks)"))
            ]
            for col_name, col_def in new_cols.items():
    ...
    try:
        with engine.connect() as conn:
            ...
            if "email_accounts" not in tables:
                return
            existing = (
                conn.execute(text("SELECT COUNT(*) FROM email_accounts")).scalar() or 0
            )
            if existing > 0:
                return
...
def _migrate_encrypt_endpoint_keys():
    ...
    try:
        with engine.connect() as conn:
            rows = conn.execute(
                text("SELECT id, api_key FROM model_endpoints")
            ).fetchall()
            migrated = 0
            for rid, key in rows:
...
def _migrate_encrypt_signatures():
    ...
    try:
        with engine.connect() as conn:
            rows = conn.execute(
                text("SELECT id, data_png, svg FROM signatures")
            ).fetchall()
            migrated = 0
