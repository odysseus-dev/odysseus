...
class GalleryImage(TimestampMixin, Base):
    ...
    # Generation provenance (resolved params from scheduler)
    gen_seed           = Column(Integer, nullable=True)
    ...
    gen_style          = Column(String, nullable=True)
    gen_op             = Column(String, nullable=True)
    source_image_id    = Column(String, nullable=True)
    loras_json         = Column(Text, nullable=True)
...
def init_db():
    ...
    _migrate_add_gallery_provenance_columns()
    _migrate_backfill_task_folders()


def _migrate_add_gallery_provenance_columns():
    """Add image generation provenance columns to gallery_images."""
    import sqlite3
    db_path = DATABASE_URL.replace("sqlite:///", "")
    if not os.path.exists(db_path):
        return
    ...
    try:
        ...
        for col, typ in additions:
            if col not in existing:
                ...
                logging.getLogger(__name__).info("Migrated: added gallery_images.%s", col)
        conn.commit()
    except Exception as e:
        logging.getLogger(__name__).warning("gallery provenance migration failed: %s", e)
    finally:
        if conn:
            try:
                conn.close()
