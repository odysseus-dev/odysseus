...
def rank_search_results(query: str, results: List[dict]) -> List[dict]:
    ...
    def title_score(title: str) -> float:
        ...
        matches = sum(
            1 for term in query_terms if re.search(rf"\b{re.escape(term)}\b", title_lc)
        )
    ...
    def news_quality_adjustment(title: str, snippet: str, url: str) -> float:
        ...
        if netloc in _LOW_VALUE_NEWS_DOMAINS:
            adjustment -= 0.8
        if not is_sports_query and any(
            hint in text or hint in netloc for hint in _SPORTS_HINTS
        ):
            adjustment -= 1.5
