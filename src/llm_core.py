...
def _ollama_normalize_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt Void' canonical OpenAI-style messages to native Ollama /api/chat.

    Two shape mismatches silently break requests:

    1. Tool calls: Void carries `function.arguments` as a JSON *string*.
    ...
    2. Images (issue #4723): Void carries multimodal user content as an
...
def _apply_local_cache_affinity(payload: Dict, url: str, session_id: Optional[str]) -> None:
    ...
    Sending a stable ``session_id`` (derived from the Void session) lets
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        h.setdefault("HTTP-Referer", "https://github.com/MKarthik730/void-core")
        h.setdefault("X-OpenRouter-Title", "Void")
    if provider == "copilot":
...
# value returns HTTP 400. Void does not currently send the `thinking` mode
...
# VOID_MISTRAL_REASONING_EFFORT (e.g. set to "medium" for cheaper chat).
_MISTRAL_REASONING_EFFORT = os.getenv("VOID_MISTRAL_REASONING_EFFORT", "high")
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip Void-only metadata before sending messages to providers.
...
async def stream_llm(url: str, model: str, messages: List[Dict], temperature: float = LLMConfig.DEFAULT_TEMPERATURE,
    ...
    if provider == "anthropic":
        ...
        # Effort level is configurable via VOID_MISTRAL_REASONING_EFFORT
