...
async def action_consolidate_memory(owner: str, **kwargs) -> Tuple[str, bool]:
    ...
    try:
                ...
                if start != -1 and end != -1 and end > start:
                    ...
                    if isinstance(keep_items, list) and isinstance(drop_items, list):
                        ...
                        if keep_ids:
                            ...
                            raise TaskNoop(
                                f"AI scanned {len(owner_memories)} memories, no changes"
                            )
            except TaskNoop:
                raise
        ...
        extra = (
            f" (+{removed - len(removed_examples)} more)"
            if removed > len(removed_examples)
            else ""
        )
        return (
            f"Removed {removed} duplicate(s) of {len(owner_memories)}: {preview}{extra}",
            True,
        )
    except Exception as e:
...
async def action_mark_email_boundaries(owner: str, **kwargs) -> Tuple[str, bool]:
    ...
    try:
        ...
        already = {
            r[0]
            for r in c.execute("SELECT message_id FROM email_boundaries").fetchall()
        }
        c.close()
                ...
                try:
                    ...
                    if (bulkish or marketingish) and score < 2:
                        ...
                        if not reason or "urgent" in reason.lower():
                            reason = (
                                "Bulk marketing/newsletter; no personal reply needed"
                            )
