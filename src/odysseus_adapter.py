...
def install_memory_platform(memory_manager, memory_vector,
    ...
    # ---- 6. drift protection — activity-gated, like the pressure mechanic ----
    # The drift check is only meaningful when the store has actually changed
    # since the last check (a fixed blind timer would burn cycles for nothing).
    # So the watchdog:
    #   - polls on a short cadence (cheap: one COUNT query),
    #   - runs the full drift check ONLY when entries changed since the last
    #     check (the same "only act when there is something to act on" signal
    #     that gates sleep on pressure),
    #   - self-heals with autofix=True, and logs when drift survives autofix.
    _dledger = platform.get("drift_ledger")
    if _dledger is not None:
        try:
            DRIFT_POLL_SEC = int(os.environ.get("MEMORY_DRIFT_POLL_SEC", "60"))
            _pstore = platform.get("store")
            _pconn = platform.get("store_db")
            _state = {"last_count": None, "checked_at": 0.0}
            def _drift_watchdog():
                while True:
                    time.sleep(DRIFT_POLL_SEC)
                    try:
                        changed = True
                        if _pstore is not None and _pconn is not None:
                            try:
                                n = _pstore.stats(_pconn).get("active_entries")
                                changed = (n != _state["last_count"])
                                _state["last_count"] = n
                            except Exception:
                                changed = True
                        if not changed:
                        ...
                        if not ok:
                            logger.warning("DRIFT-WATCHDOG: core block drift "
                                           "detected after autofix (unanchored "
                                           "change)")
                    except Exception as e:
