...
async def stream_agent_loop(
    ...
    for round_num in range(1, max_rounds + 1):
        all_tool_schemas = select_turn_tool_schemas(
        ...
        _turn = ModelTurn(ModelTurnRequest(
            round_num=round_num,
            ...
            deadline=time.time() + max(agent_stream_timeout * 4, 1200),
            requested_model=requested_model,
            actual_model=actual_model,
            total_start=total_start,
            first_token_received=first_token_received,
            tool_policy=tool_policy,
            stream_llm=stream_llm_with_fallback,
        ))
        async for chunk in _turn.stream():
            yield chunk

        _turn_result = _turn.result
        _turn_usage = _turn_result.usage
        if _turn_usage:
            actual_model = _turn_usage.actual_model
            real_input_tokens += _turn_usage.input_tokens
            real_output_tokens += _turn_usage.output_tokens
            if _turn_usage.last_round_input_tokens:
                last_round_input_tokens = _turn_usage.last_round_input_tokens
            has_real_usage = has_real_usage or _turn_usage.has_real_usage
            if _turn_usage.backend_gen_tps:
                backend_gen_tps = _turn_usage.backend_gen_tps
            if _turn_usage.backend_prefill_tps:
                backend_prefill_tps = _turn_usage.backend_prefill_tps
            if _turn_usage.time_to_first_token is not None and time_to_first_token is None:
                time_to_first_token = _turn_usage.time_to_first_token
            first_token_received = first_token_received or _turn_usage.first_token_received

        round_response = _turn_result.round_response
        round_reasoning = _turn_result.round_reasoning
        native_tool_calls = _turn_result.native_tool_calls
        tool_blocks = _turn_result.tool_blocks
        used_native = _turn_result.used_native
        full_response += _turn_result.response_text
        ...
        _tools = ToolTurn(ToolTurnRequest(
            ...
            doc_stream_started=_turn_result.doc_stream_started or bool(_auto_doc_events),
            execute_tool=execute_tool_block,
            format_tool_result=format_tool_result,
        ))
        async for chunk in _tools.stream():
            yield chunk
        _tool_result = _tools.result
        total_tool_calls = _tool_result.total_tool_calls
        full_response += _tool_result.response_text
        tool_events.extend(_tool_result.tool_events)
        if _tool_result.effectful_used:
            _effectful_used = True
        if _tool_result.awaiting_user:
            _awaiting_user = True
        tool_results = _tool_result.tool_results
        tool_result_texts = _tool_result.tool_result_texts

        # If budget was hit, stop the loop
        if _tool_result.budget_hit:
            break
