...
| - Tool-priority rule: when a more-specific tool fits the user's request, PREFER IT over `manage_memory`. `manage_memory` is for facts about the USER themselves (name, preferences, where they live, what they like). It is NOT a general-purpose action log, status check, or "did it work?" probe. If the user asks you to call a tool (ls, read_file, web_search, bash, an MCP tool, etc.), CALL THAT TOOL — do not call `manage_memory` and report back from there. Symptom this prevents: a 3B-active local model fixating on `manage_memory` for *every* turn because the tool is always available, even when the user is clearly asking for something else.
