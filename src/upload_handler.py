...
class UploadHandler:
    def __init__(self, base_dir: str, upload_dir: str):
        ...
        self._index_signature: Optional[tuple[tuple[str, int, int], ...]] = None
        ...
        *,
        ...
        if path.endswith("uploads.json"):
            ...
            try:
                ...
                self._index_signature = tuple(
                    (candidate, candidate_stat.st_mtime_ns, candidate_stat.st_size)
                    for candidate in (path, path + ".bak")
                    if os.path.exists(candidate)
                    for candidate_stat in (os.stat(candidate),)
                )
            except OSError:
    ...
    def _load_upload_index(self, *, fail_on_error: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                and stats == self._index_signature
            ):
