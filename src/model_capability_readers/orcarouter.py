"""OrcaRouter model catalog capability reader.

OrcaRouter is an OpenAI-compatible AI gateway that exposes the same provider
model namespace as OpenRouter (``/api/v1/models`` with rich per-model records).
Like the OpenRouter reader, this normalizes architecture/modality and
supported-parameter fields into capability records so routed gateway models
show real capabilities instead of identity-only records.
"""
...
def record_from_model(
    ...
    *,
    ...
    return ModelCapabilityRecord(
        vendor=VENDOR_ORCAROUTER,
        model_id=model_id,
        stable_model_id=stable_model_id_for(VENDOR_ORCAROUTER, model_id, endpoint_id=endpoint_id, base_url=base_url),
