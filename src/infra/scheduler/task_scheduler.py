...
def _resolve_task_timezone(db, task) -> str | None:
    ...
    if not getattr(task, "crew_member_id", None):
        return None
    try:
        from src.infra.database.database import CrewMember
...
class TaskScheduler:
    ...
    def _set_run_progress(self, run_id: str, message: str):
        ...
        if not run_id:
            return
        try:
            from src.infra.database.database import SessionLocal, TaskRun
            db = SessionLocal()
            try:
    ...
    async def start(self):
        ...
        try:
            from src.infra.database.database import SessionLocal as _SL, ScheduledTask as _ST
            db = _SL()
            try:
                now = _utcnow()
    ...
    def _known_task_owners(self) -> list:
        ...
        """
        from src.infra.database.database import SessionLocal, ScheduledTask, Note
        db = SessionLocal()
        try:
            owners = set()
    ...
    async def _execute_checkin(self, task, crew, db, session_id: str,
        ...
        try:
            from src.infra.database.database import SessionLocal as _SL, CalendarEvent as _CE
            _db = _SL()
            try:
    ...
    async def _execute_llm_task(self, task, db) -> str:
        ...
        from src.infra.database.database import Session as DbSession, ChatMessage, CrewMember
    ...
    async def _run_agent_loop(self, endpoint_url: str, model: str, task, session_id: str,
        ...
        try:
            from src.infra.llm.endpoint_resolver import resolve_utility_fallback_candidates
    ...
    def _has_chain_cycle(self, db, start_id: str, max_depth: int = 10, owner: str | None = None) -> bool:
        ...
        from src.infra.database.database import ScheduledTask
        visited = set()
        current = start_id
    ...
    async def ensure_defaults(self, owner: str):
        ...
        try:
            ...
            from src.infra.database.database import TaskRun
            retired_ids = [
