...
Model-serving / cookbook tool implementations extracted from
tool_implementations.py (P8-T4): the internal-loopback API client (do_app_api),
llama.cpp/cookbook server lifecycle (serve/stop/tail/list/adopt), model
download/cancel/search, preset serving, cached-model listing and image editing.
The internal base URL is resolved exactly as before via
core.constants.internal_api_base() — no host literal is hard-coded here.
Shared _parse_tool_args is reached through the tool_implementations module
object so any test patch on it still binds.
"""
...
async def do_app_api(content: str, owner: Optional[str] = None) -> Dict:
    ...
    import httpx
    try:
        args = _ti._parse_tool_args(content) if content.strip() else {}
    except ValueError:
