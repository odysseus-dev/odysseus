        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "calendar_href": {"type": "string", "description": "Target calendar for create_event. Accepts the calendar id, an id prefix, or the calendar name (case-insensitive) — not only a URL. When omitted the event goes to the owner's FIRST calendar, which is often a local one that is never pushed to any remote server: the event is then created successfully but never reaches the user's CalDAV/Google calendar. Call list_calendars and pass the intended calendar explicitly. A value that matches nothing silently falls back to the same default."},
                    "calendar": {"type": "string", "description": "Calendar name, id or href. Filters list_events; also accepted by create_event as an alias of calendar_href."},
