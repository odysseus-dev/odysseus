...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "edit_file": "Exact-string find-and-replace inside an existing file on disk. The primary way to edit a file's source: change a line, fix a bug, rename, insert code. Prefer over rewriting the whole file.",
    "glob": "Find files on disk by glob pattern (e.g. **/*.py), newest first. Locate files by name or path before reading or editing them.",
    "grep": "Search file contents with a regex across the project. Find where a function, string, symbol, or pattern lives. Returns matching files, lines, or counts.",
