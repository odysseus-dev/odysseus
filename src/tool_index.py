...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    "bash": "Run shell commands through the server-owned process boundary. Sandbox is the default and confines commands to the selected or default agent workspace; only explicitly confirmed Full Access can reach the service-user filesystem. The effective process network is networkless with Web off or brokered HTTP(S) only with Web on. Install packages, run git operations and builds, inspect system info, or manage processes. Prefer a dedicated tool whenever one fits the job (file read/write/edit, search, listing); use bash only for what no dedicated tool covers. Do not use for web lookup/search; use web_search or web_fetch when web tools are available.",
    "python": "Execute Python code with the application's dependency environment through the server-owned process boundary. Sandbox is the default and confines code to the selected or default agent workspace; only explicitly confirmed Full Access can reach the service-user filesystem. The effective process network is networkless with Web off or brokered HTTP(S) only with Web on. Use for computation, data processing, math, scripting, and parsing, not for writing code for the user. Prefer a dedicated tool for reading, writing, or searching files; use python only for what no dedicated tool covers. Do not use for web lookup/search; use web_search or web_fetch when web tools are available.",
    ...
    "get_workspace": "Return the active workspace and effective process boundary. File tools are confined to the workspace. Bash, Python, and detached processes use Sandbox by default and are confined there; only explicitly confirmed Full Access treats it as a starting directory while retaining service-user filesystem access. The result reports the effective network profile: networkless or brokered HTTP(S) only. Call this first when the user refers to 'the project'/'the code'/'this folder' without giving a path, instead of asking them.",
