...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "ask_user": "Ask the user one multiple-choice question, or a batch of 1-4 questions, to get a decision or clarification. Use this when the task is genuinely ambiguous and the answer changes what you do next — pick between approaches, confirm an assumption, choose among options — instead of guessing. Legacy calls provide a clear `question` and 2-6 `options`; batched calls provide `questions` entries with `question`, optional short `header`, 2-4 `options`, and optional `multiSelect`. Calling this ENDS your turn: the user sees clickable buttons and their choice arrives as your next message. Don't use it for things you can decide from context or sensible defaults, or for irreversible-action confirmation if a dedicated flow exists.",
