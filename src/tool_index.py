...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "manage_atlas": "Read/write/query the user's Atlas markdown vault — an Obsidian-style folder of .md notes with [[wikilinks]], #tags, frontmatter properties, backlinks and a link graph. action='list'/'read'/'write'/'append'/'delete'/'search'/'backlinks'/'graph'/'query'. action='query' runs a structured Bases query over properties and heading sections (e.g. 'todo sections where status is open'). Use [[Note name]] in content to link notes together. Use this for the user's personal knowledge base / 'my vault / my notes / my zettelkasten / second brain' — distinct from the Documents editor (manage_documents) and Keep-style notes (manage_notes).",
