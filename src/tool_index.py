...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    "bash": "Run shell commands on the server. Install packages, check files, git operations, curl, system info, process management, networking. Do not use for web lookup/search; use web_search or web_fetch when web tools are available.",
    "powershell": "Run a PowerShell script natively (pwsh / powershell.exe). Use on Windows for system tasks, file management, registry, environment variables, WMI, .NET APIs. Not bash — use PowerShell syntax.",
    "python": "Execute Python code for computation, data processing, math, scripting, parsing, API calls. Not for writing code for the user. Do not use for web lookup/search; use web_search or web_fetch when web tools are available.",
