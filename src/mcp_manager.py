...
class McpManager:
    ...
    async def connect_server(
        ...
        try:
            if transport == "stdio":
                ...
                res = await self._connect_sse(server_id, name, url, headers or {})
            elif transport == "http":
                res = await self._start_http_connect(server_id, name, url, headers or {})
            else:
    ...
    async def _connect_sse(self, server_id: str, name: str, url: str, headers: Dict[str, str]) -> bool:
    ...
    async def _start_http_connect(self, server_id: str, name: str, url: str, headers: Optional[Dict[str, str]] = None, wait: float = 8.0) -> bool:
        ...
        task = asyncio.create_task(self._connect_http(server_id, name, url, headers or {}))
    ...
    async def _connect_http(self, server_id: str, name: str, url: str, headers: Dict[str, str]) -> bool:
        ...
        try:
            ...
            stack = AsyncExitStack()
            # Static headers (for example a Portainer MCP gate token) are a
            # complete auth posture. Do not let OAuth discovery replace them.
            provider = None if headers else build_provider(server_id, url, on_redirect=_on_redirect)
            if headers:
