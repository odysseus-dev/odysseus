...
def test_fallback_emits_indicator_when_primary_fails(monkeypatch):
    ...
    fb = [
        json.loads(c[6:])
        for c in chunks
        if c.startswith("data: ") and '"fallback"' in c
    ]
    ...
    order = [
        i for i, c in enumerate(chunks) if '"fallback"' in c or '"delta": "hello"' in c
    ]
...
def test_dedupe_candidates_keeps_first_of_each_route():
    ...
    assert llm_core._dedupe_candidates(cands) == [
        ("u1", "m1", {"h": 1}),
        ("u2", "m2", {}),
    ]
...
def test_duplicate_route_is_attempted_only_once(monkeypatch):
    ...
    async def run():
        ...
        async for c in llm_core.stream_llm_with_fallback(
            cands, [{"role": "user", "content": "hi"}]
        ):
            out.append(c)
        return out

    asyncio.run(run())
    assert calls == [("u1", "m1"), ("u2", "m2")], (
        f"duplicate route re-attempted: {calls}"
    )
