...
def test_rewrite_reformulates_question(monkeypatch):
    _patch_llm(monkeypatch, "Albert Einstein death date")
    out = _run(q.rewrite_search_query("is albert einstein still alive?"))
    assert out == "Albert Einstein death date"
...
def test_rewrite_falls_back_on_llm_error(monkeypatch):
    ...
    original = "is albert einstein still alive?"
