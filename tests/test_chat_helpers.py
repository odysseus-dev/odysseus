...
def test_allowed_models_legacy_empty_list_remains_unrestricted(monkeypatch):
    monkeypatch.setattr("src.api.handler.chat_helpers.get_current_user", lambda request: "alice")
...
def test_admin_user_is_never_blocked(monkeypatch):
    ...
    monkeypatch.setattr("src.api.handler.chat_helpers.get_current_user", lambda request: "admin")
