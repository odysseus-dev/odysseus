...
def _compose_env_names(path: Path) -> set[str]:
    ...
    env = compose["services"]["void"]["environment"]
...
def _upload_limit_env_names() -> set[str]:
    ...
    return set(re.findall(r'"(VOID_[A-Z_]*BYTES)"', source)) | {
        "VOID_CHAT_UPLOAD_MAX_BYTES"
    }
