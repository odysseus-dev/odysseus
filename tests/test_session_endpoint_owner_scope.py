...
def test_chat_endpoint_recovery_paths_are_owner_scoped():
    ...
    # NOTE: ruff format may wrap long signatures across lines, so assert on the
    # function name + the owner-scoping parameter independently (behavior-preserving).
    assert "def _clear_orphaned_session_endpoint(" in chat_routes
    assert "def _recover_empty_session_model(" in chat_routes
    assert "session_id: str, owner:" in chat_routes
