...
_MISSING = object()
_MODULE_NAMES = ("core", "src.infra.database.database")
...
def test_models_create_mock_attributes(monkeypatch):
    ...
    assert sys.modules["src.infra.database.database"] is db
...
def test_undo_removes_modules_that_were_absent():
    with _preserve_core_modules():
        ...
        try:
            ...
            assert "src.infra.database.database" in sys.modules
        finally:
            monkeypatch.undo()
...
def test_undo_restores_existing_modules():
    with _preserve_core_modules():
        original_core = ModuleType("core")
        original_database = ModuleType("src.infra.database.database")
        sys.modules["core"] = original_core
        sys.modules["src.infra.database.database"] = original_database
        monkeypatch = MonkeyPatch()
        try:
            make_core_db_stub(monkeypatch, install_core_package=True)

            assert sys.modules["core"] is not original_core
            assert sys.modules["src.infra.database.database"] is not original_database
        finally:
            monkeypatch.undo()

        assert sys.modules["core"] is original_core
        assert sys.modules["src.infra.database.database"] is original_database
