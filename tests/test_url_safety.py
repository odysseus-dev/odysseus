...
LOOPBACK_V6 = _resolver({"localhost": ["::1"]})
...
def test_loopback_and_lan_allowed_by_default_local_first():
    ...
    assert check_outbound_url("http://localhost:8080/v1", resolver=LOOPBACK_V6)[0] is True
...
def test_strict_mode_blocks_private_and_loopback():
    ...
    ok, reason = check_outbound_url("http://localhost:8080", block_private=True, resolver=LOOPBACK_V6)
    assert ok is False and "loopback" in reason
