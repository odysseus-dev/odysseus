...
from src.agent_tools.ui_tools import UIControlTool


def test_toggle_rag_on_is_accepted():
    r = asyncio.run(UIControlTool().execute("toggle rag on", {}))
...
def test_toggle_rag_off_is_accepted():
    r = asyncio.run(UIControlTool().execute("toggle rag off", {}))
    assert r.get("toggle_name") == "rag"
...
def test_unknown_toggle_still_rejected():
    r = asyncio.run(UIControlTool().execute("toggle bogus on", {}))
    assert "error" in r


def test_existing_toggle_still_works():
    r = asyncio.run(UIControlTool().execute("toggle web on", {}))
