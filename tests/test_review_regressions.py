...
def _install_model_route_import_stubs(monkeypatch):
    ...
    monkeypatch.delitem(sys.modules, "src.api.router.model_routes", raising=False)
    ...
    # model_routes.py imports "from core.middleware import require_admin".
    # If core.middleware is cached from a previous test it carries the real
    # require_admin, which rejects our bare SimpleNamespace request.  Delete
    # the cached entry so the stubbed middleware_mod wins on re-import.
    monkeypatch.delitem(sys.modules, "core.middleware", raising=False)
...
async def test_webhook_tool_reuses_private_url_validation():
    ...
    _ABSENT = object()
    _wm_saved_module = sys.modules.get("src.infra.scheduler.webhook_manager", _ABSENT)
    ...
    sys.modules.pop("src.infra.scheduler.webhook_manager", None)
    ...
    try:
        ...
        if _wm_saved_module is _ABSENT:
            ...
            sys.modules["src.infra.scheduler.webhook_manager"] = _wm_saved_module
