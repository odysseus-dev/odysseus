...
def test_llm_call_async_thinking_only_payload_returns_thinking(monkeypatch):
    """No text block yet: thinking alone must still come back as a string."""
    out = _call(monkeypatch, [
        {"type": "thinking",
         "thinking": [{"type": "text", "text": "Only reasoning so far"}],
         "closed": True},
    ])
    assert isinstance(out, str)
    assert "Only reasoning so far" in out


def test_llm_call_async_plain_string_content_unchanged(monkeypatch):
    """Non-Mistral providers return string content; it must pass through as-is."""
