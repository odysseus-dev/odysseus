...
def test_trusted_egress_helpers_are_installed_root_owned_by_dockerfile():
    ...
    assert "make -C security/egress install" in dockerfile
    assert "INSTALL_OWNER ?= root" in makefile
    assert "INSTALL_GROUP ?= root" in makefile
    assert 'realpath "$$(command -v $(PYTHON))"' in makefile
    assert "install -o $(INSTALL_OWNER) -g $(INSTALL_GROUP) -m 0755" in makefile
    assert "sed -i '1c\\#!$(PYTHON_PATH) -I'" in makefile
    assert "$(INSTALL_DIR)/odysseus-egress-broker" in makefile
    assert "$(INSTALL_DIR)/odysseus-egress-bridge" in makefile
