...
def test_manual_rocm_and_cuda_are_not_unified_memory():
    for backend in ("cuda", "rocm"):
        s = _apply_manual_hardware(
            {"unified_memory": True}, manual_mode="gpu", manual_backend=backend
        )
...
def test_manual_ram_mode_wipes_gpu_and_unified_flag():
    s = _apply_manual_hardware(
        {"unified_memory": True}, manual_mode="ram", manual_ram_gb="64"
    )
...
def test_simulated_metal_box_only_recommends_gguf():
    ...
    assert unservable == [], (
        f"{len(unservable)} non-GGUF models on simulated Metal, e.g. {unservable[:3]}"
    )
