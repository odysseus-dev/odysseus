...
def test_no_hardcoded_portal_z_literals_remain_in_css():
    # style.css was split into modular files under static/css/; assert against
    # the same concatenated bundle the browser resolves via index.css imports.
    from tests.helpers.css_loader import load_runtime_css_text

    src = load_runtime_css_text()
    hits = re.findall(r"z-index:\s*10000[02](?!\d)", src)
    assert not hits, f"runtime CSS still has hardcoded portal z: {hits}"
