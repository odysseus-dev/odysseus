...
def _load_setup_module():
    spec = importlib.util.spec_from_file_location("void_setup_under_test", Path("setup.py"))
...
def test_create_default_admin_normalizes_env_username(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("VOID_ADMIN_USER", " AdminUser ")
    monkeypatch.setenv("VOID_ADMIN_PASSWORD", "temporary-password")
...
def test_main_loads_admin_password_from_env_file(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("VOID_ADMIN_USER", raising=False)
    monkeypatch.delenv("VOID_ADMIN_PASSWORD", raising=False)
    (tmp_path / ".env").write_text(
        "VOID_ADMIN_USER=presetuser\nVOID_ADMIN_PASSWORD=fromenvfile12345\n",
        encoding="utf-8-sig",
    )
    ...
    monkeypatch.setenv("VOID_SKIP_ADMIN_PROMPT", "1")
    ...
    try:
        ...
        # don't inherit them.
        os.environ.pop("VOID_ADMIN_USER", None)
        os.environ.pop("VOID_ADMIN_PASSWORD", None)
