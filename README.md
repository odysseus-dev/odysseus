...
| `ODYSSEUS_CODEX_MODEL_PROVIDER_ENABLED` | `false` | Enables the experimental Add Models -> OpenAI subscription provider backed by `codex exec`. |
...
### OpenAI Subscription Model Provider

The sign-in above can optionally become a shared text LLM source. Set `ODYSSEUS_CODEX_MODEL_PROVIDER_ENABLED=true`, restart Odysseus, sign in under **Settings -> Integrations -> Codex / ChatGPT**, then open **Settings -> Add Models -> OpenAI subscription** and click **Add**.
...
- It plugs into the same shared LLM resolver as API-key endpoints, so Default, Utility, Research, task, email, document, and naming flows can use it when selected.
- It is text-LLM only, non-streaming, and starts a fresh `codex exec` process for each request.
