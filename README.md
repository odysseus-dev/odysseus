...
Odysseus also ships a built-in YoutubeDownloader MCP server that talks to the local YoutubeDownloader web service. The YoutubeDownloader service must be installed and running on the same device as Odysseus. It uses `http://127.0.0.1:49156` by default; set `YTDL_BASE_URL` if your [YoutubeDownloader](https://github.com/LorenBll/YoutubeDownloader) service is running elsewhere.
