...
# Keys copied from SSE tool_output payloads into persisted tool_events metadata.
_TOOL_EVENT_EXTRA_KEYS = (
    "image_url", "image_id", "image_prompt", "image_model", "image_size",
    "image_quality", "gallery_id", "image_urls", "diff", "screenshot",
    "wizard_pending", "pending_user", "trigger_mode", "image_proposal",
    "seed", "provenance", "args",
)


def sse_payload_to_tool_event(data: dict, *, round_num: int = 1) -> dict:
    """Build a tool_events entry from a streamed tool_output SSE payload."""
    ev: dict[str, Any] = {
        "round": round_num,
        "tool": data.get("tool"),
    ...
    for key in _TOOL_EVENT_EXTRA_KEYS:
        if data.get(key) is not None:
            ev[key] = data[key]
    return ev


def turn_has_saveable_content(
    full_response: str,
    metrics: dict | None,
    stream_tool_events: list | None,
) -> bool:
    """True when an assistant turn should be persisted despite empty prose."""
    if (full_response or "").strip():
        return True
    if stream_tool_events:
        return True
    if metrics and metrics.get("tool_events"):
        return True
    return False


def merge_turn_save_metrics(
    metrics: dict | None,
    *,
    stream_tool_events: list | None = None,
    requested_model: str = "",
    actual_model: str = "",
    stopped: bool = False,
) -> dict:
    """Merge streamed tool events into metrics before save_assistant_response."""
    md = dict(metrics or {})
    if stream_tool_events and not md.get("tool_events"):
        md["tool_events"] = stream_tool_events
    if stopped:
        md["stopped"] = True
    if requested_model:
