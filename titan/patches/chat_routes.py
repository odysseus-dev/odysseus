...
from routes.chat_helpers import (
    ...
    turn_has_saveable_content,
    merge_turn_save_metrics,
    sse_payload_to_tool_event,
)
                ...
                try:
                    ...
                    async for chunk in stream_agent_loop(
                        ...
                        if chunk.startswith("data: ") and not chunk.startswith("data: [DONE]"):
                            try:
                                ...
                                if "delta" in data:
                                    ...
                                    if data.get("type") == "agent_step":
                                        ...
                                        _stream_tool_events.append(
                                            sse_payload_to_tool_event(
                                                data, round_num=max(_agent_rounds, 1)
                                            )
                                        )
                                    yield chunk
                                elif data.get("type") == "fallback":
                            ...
                            if turn_has_saveable_content(
                                full_response, last_metrics, _stream_tool_events
                            ):
                                _save_metrics = merge_turn_save_metrics(
                                    last_metrics,
                                    stream_tool_events=_stream_tool_events,
                                    requested_model=_requested_model,
                                    actual_model=_actual_model or _answered_by or _requested_model,
                                )
                                _saved_id = save_assistant_response(
                                    sess, session_manager, session, full_response, _save_metrics,
                                    character_name=ctx.preset.character_name,
                                    web_sources=web_sources,
                                    rag_sources=ctx.rag_sources,
                                    used_memories=ctx.used_memories,
                                ...
                                run_post_response_tasks(
                                    sess, session_manager, session, message, full_response,
                                    _save_metrics, ctx.uprefs, memory_manager, memory_vector, webhook_manager,
                                    incognito=incognito, compare_mode=compare_mode,
                                    character_name=ctx.preset.character_name,
                    ...
                    try:
                        if turn_has_saveable_content(
                            ...
                            logger.info(
                                "Client disconnected mid-stream for session %s, saving partial "
                                "(%d chars, %d tool events)",
                                session, len(full_response), len(_stream_tool_events),
                            )
                            _save_metrics = merge_turn_save_metrics(
                                last_metrics,
                                stream_tool_events=_stream_tool_events,
