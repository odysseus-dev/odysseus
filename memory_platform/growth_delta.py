...
OLLAMA = memory_env.ollama_url()
# Configurable via settings (growth_delta_model key) or env var.
DEFAULT_DELTA_MODEL = os.environ.get("GROWTH_DELTA_MODEL", "qwen2.5:3b")


def _get_delta_model():
    """Get the configured growth delta model from settings."""
    try:
        from src.settings import get_setting
        return get_setting("growth_delta_model", DEFAULT_DELTA_MODEL)
    except Exception:
        return DEFAULT_DELTA_MODEL
...
def _llm(prompt):
    ...
    payload = json.dumps({
        "model": _get_delta_model(), "prompt": prompt, "stream": False,
        "think": False,
